sbjs = ["S12_33_DA";"S12_34_TC";"S12_35_LM";"S12_36_SrS";"S12_38_LK";"S12_39_RT";...
    "S12_40_MJ";"S12_41_KS";"S12_42_NC";"S12_45_LR";"S13_46_JDB";"S13_47_JT2";...
    "S13_50_LGM";"S13_51_MTL";"S13_52_FVV";"S13_53_KS2";"S13_54_KDH";"S13_56_THS";...
    "S13_57_TVD";"S13_59_SRR";"S13_60_DY";"S14_62_JW";"S14_66_CZ";"S14_67_RH";...
    "S14_74_OD";"S14_75_TB";"S14_76_AA";"S14_78_RS";"S15_81_RM";"S15_82_JB";...
    "S15_83_RR";"S15_87_RL";"S16_95_JOB";"S16_96_LF"];
proj = "lbcn";
task = "MMR";


%% Main
trialNfoA = cell(numel(sbjs),1); % preallocate

% Subject loop
for s = 1:numel(sbjs)
    trialNfo = ec_loadSbj(sbj=sbjs(s),proj=proj,task=task,hz=1000,vars="trialNfo");
    trialNfo.Properties.RowNames =...
        "s"+trialNfo.sbjID+"_t"+trialNfo.tr+"_"+trialNfo.cond;
    trialNfoA{s} = trialNfo;
end


%% Finalize
trialNfoA = vertcat(trialNfoA{:});

fn = "/01/lbcn/anal/trialNfoA_"+string(datetime('now','TimeZone','local','Format','yyMMdd'));
save(fn,"trialNfoA","-v7"); disp("SAVED: "+fn)