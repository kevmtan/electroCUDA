...
o.hiPass = "ASR"; % Hi-pass cutoff in hertz (skip=0)
...
o.detrendItr = [1 3]; %[10 2]; % number of iterations [iterChunkedRun iterEntireRun] {default=[10 2]}
o.detrendThr = [5 3]; %[6 3]; % outlier threshold [threshChunkedRun threshEntireRun] {default=[6 3]}
...
% ASR
o.asr.do = true;
o.asr.hiPass = [0.25 0.75];
o.asr.flat = 5; % Flatline criterion
o.asr.chCorr = 0.8; % Channel (correlation) criterion
o.asr.chMaxBadTime = 0.5;
o.asr.noLocs = 0.45; % NoLocsChannelCriterion
o.asr.noLocsExcluded = [];  % NoLocsChannelCriterionExcluded
o.asr.line  = 4; % LineNoiseCriterion
o.asr.burst = 20; % BurstCriterion
o.asr.burstRej {mustBeMember(o.asr.burstRej,["on" "off"])} = 'off';
o.asr.burstMaxBadChs = 0.075;
o.asr.burstTols = [-inf 5.5];
o.asr.win = 0.25; % Window criterion
o.asr.winTols = [-inf 7];
o.asr.dist {mustBeMember(o.asr.dist,["riemannian" "euclidian"])} = 'euclidian';
o.asr.maxMem = []; %ec_ramAvail;
o.asr.chInclude = [];
o.asr.chIgnore = [];
o.asr.numSamples = 50;
o.asr.gpu = true;
...
for s = 1:height(statusPP) % subject loop
    ...
    if doICA && statusPP.ICA(s)~=1
        ...
        oICA.hiPassICA = 1; % Hi-pass cutoff in hertz
        oICA.hiPass = 0;
        oICA.detrendOrder_ICA = []; % polynomial order (suggested=10, skip=0)
        oICA.detrendThr_ICA = [];
        oICA.detrendItr_ICA = [];
        oICA.detrendOrder = [];
        oICA.detrendThr = [];
        ...
        try disp([sbj ': starting ICA']); clear n xi;
            [errICA,n,xi] = ec_preprocICA(sbj,proj,oICA,save=1,dirs=dirs);
    ...
    if doICAspec && statusPP.spec(s)~=1
        ...
        % Run
        try disp("Starting spec: "+sbj);
            errSpec = ec_preprocTimeFreq(sbj,proj,oSpec,n,xi,save=1,ica=1,dirs=dirs);
            if nnz(~cellfun(@isempty,errSpec)); statusPP.errorSpec{s}=errSpec; end
            statusPP.spec(s) = 1; disp("Finished spec: "+sbj);
            try reset(gpuDevice(1)); catch;end
        catch ME; warning("ERROR spec: "+sbj); getReport(ME)
    ...
    if doICAhfb && statusPP.HFB(s)~=1
        ...
        % Run
        try disp("Starting HFB: "+sbj);
            errHFB = ec_preprocTimeFreq(sbj,proj,oHFB,n,xi,save=1,ica=1,dirs=dirs);
            if nnz(~cellfun(@isempty,errHFB)); statusPP.errorHFB{s}=errHFB; end
            statusPP.HFB(s) = 1; disp("Finished HFB: "+sbj);
        catch ME; warning("ERROR HFB: "+sbj); getReport(ME)
            statusPP.errorHFB{s} = {lastwarn,ME};
...
for s = 1:height(statusPP) % Subject loop
    ...
    oICA.detrendOrder = [0 0];
    oICA.detrendThr = [10 3];
    ...
    % Run
    try disp("Starting LFP: "+sbj);
        [errPreproc,n,x] = ec_preproc(sbj,proj,oICA,save=0,dirs=dirs,redoN=1);
        if nnz(~cellfun(@isempty,errPreproc)); statusPP.errorLFP{s}=errPreproc; end
        statusPP.LFP(s) = 1; disp("Finished LFP: "+sbj);
    catch ME; warning("ERROR LFP: "+sbj); getReport(ME)
        clear n x
    ...
    % Run
    try disp("Starting HFB: "+sbj);
        [errHFB,n,x] = ec_preprocTimeFreq(sbj,proj,oICA,n,x,save=0,dirs=dirs,redoN=1);
        if nnz(~cellfun(@isempty,errHFB)); statusPP.errorHFB{s}=errHFB; end
        statusPP.HFB(s) = 1; disp("Finished HFB: "+sbj);
    catch ME; warning("ERROR HFB: "+sbj); getReport(ME)
        clear n x
    ...
    try disp([sbj ': starting ICA']);
        errICA = ec_preprocICA(sbj,proj,oICA,n,x,save=1,dirs=dirs,nIn=ni,redoN=1);
        if nnz(~cellfun(@isempty,errICA)); statusPP.errorICA{s}=errICA; end
