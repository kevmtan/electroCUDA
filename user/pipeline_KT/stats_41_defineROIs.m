...
dirs = ec_getDirs(proj);

cd(dirs.proc);

sbjs = dir;
sbjs = string(extractfield(sbjs,"name"));
sbjs = extractAfter(sbjs,"s");
sbjs = double(sbjs);
sbjs = sbjs(~isnan(sbjs));

chNfoA = cell(numel(sbjs),1);

%%
for s = 1:numel(sbjs)
    sbj = sbjs(s);
    disp("SUBJECT: "+sbj);
    chNfo = defineROI_lfn(sbj,proj,task);
    chNfoA{s} = chNfo;
end
...
% 
% % ATL
% idx = ismember(chNfo.Destr,[34 35 44]);
