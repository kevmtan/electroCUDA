function CreateFolders_KT(sbj_name, project_name, block_name, center, dirs, data_format,import_server)
%% Create folders LBCN
...
if ~istable(import_server)
    all_folders = dir(fullfile(dirs.server_root));
    ...
    if istable(import_server)
        ...
        if nnz(idx)>0 && nnz(idx)<2
            ...
            if strcmp(data_format, 'TDT')
                ...
                globalVar.iEEG_data_server_path = [uigetdir([dirs.server_root sbj_folder_name]) '/'];
            elseif strcmp(data_format, 'edf')
                waitfor(msgbox(['Choose server file for iEEG data of block ' block_name{bn}]));
                [FILENAME, PATHNAME] = uigetfile([dirs.server_root sbj_folder_name,'.edf'],'All Files (*.*)','MultiSelect','on');
            ...
            if ~contains(project_name, 'Rest')
                ...
                [FILENAME, PATHNAME] = uigetfile([dirs.server_root sbj_folder_name]);
    ...
    elseif strcmp(import_server, 'auto')
        ...
        if strcmp(project_name, 'Rest')
            if strcmp(data_format, 'edf')
                globalVar.iEEG_data_server_path = [dirs.server_root sbj_folder_name filesep 'Data/Rest/' [block_name{bn} '.edf']];
            elseif strcmp(data_format, 'TDT')
                globalVar.iEEG_data_server_path = [dirs.server_root sbj_folder_name filesep 'Data/Rest/' block_name{bn} filesep];
            end
            
        elseif strcmp(project_name, 'Rest_pupil')
            if strcmp(data_format, 'edf')
                globalVar.iEEG_data_server_path = [dirs.server_root sbj_folder_name filesep 'Data/Rest_pupil/' block_name{bn} filesep [block_name{bn} '.edf']];
            elseif strcmp(data_format, 'TDT')
                globalVar.iEEG_data_server_path = [dirs.server_root sbj_folder_name filesep 'Data/Rest_pupil/'  block_name{bn} filesep];
            end
        else
