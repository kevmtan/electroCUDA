...
%% Analysis template
ep = ec_epochMetadata(psy,trialNfo,ns,o); toc
if ~a.test; ep.time=[]; end

%% Spec: denoise & baseline correction
xs = ec_baselineTrials(x,n,psy,ep,o,tt); toc
...
x = ec_baselineTrials(x,n,psy1,ep,trialNfo1,o);
...
for t = 1:nTrs
    ...
    idx = idx(ismember(ep.bin(idx),lats5));
    idx = idx(ep.iPsy(idx)<=trialNfo.idxEnd(t));
    [~,ia,ib] = intersect(lats5,ep.bin(idx),'stable');
    xe(ia,:,t) = x(idx(ib),:);
...
%% Depreciated
% function anT = ec_epochMetadata(psy,trialNfo,n,o)
...
% 
% 
% function [xa,na] = ec_baselineTrials(xa,na,psy,anT,trialNfo,o)
