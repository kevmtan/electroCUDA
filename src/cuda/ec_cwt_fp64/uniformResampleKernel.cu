...
void uniformResampleAlongFirstDim(
    emxArray_real_T *cpu_xIn, real_T opts_p, real_T opts_q,
    const emxArray_real_T *cpu_opts_filterWithPadding, real_T opts_filterDelay)
{
  emxArray_real_T gpu_opts_filterWithPadding;
  emxArray_real_T gpu_xIn;
  ...
  emxInit_real_T(&cpu_yOut, 2, &cg_emlrtRTEI, true);
  emxInit_real_T(&y, 2, &dg_emlrtRTEI, true);
  emxInit_real_T(&cpu_yRow, 2, &eg_emlrtRTEI, true);
  emxInit_real_T(&b_y, 2, &w_emlrtRTEI, true);
  emxInit_real_T(&yRow, 1, &ag_emlrtRTEI, true);
  if ((!(opts_p == 1.0)) || (!(opts_q == 1.0))) {
    if ((cpu_xIn->size[0] == 1) || (cpu_xIn->size[1] == 1)) {
      ...
      if ((cpu_xIn->size[0] == 1) && (cpu_xIn->size[1] == 1)) {
        ...
        cpu_yRow->size[0] = u1;
        cpu_yRow->size[1] = 1;
        emxEnsureCapacity_real_T(cpu_yRow, b_i, &yf_emlrtRTEI);
      } else {
        upfirdn(cpu_xIn, &gpu_xIn, &xIn_outdatedOnGpu,
                (emxArray_real_T *)cpu_opts_filterWithPadding,
                &c_opts_filterWithPadding_outdat, &gpu_opts_filterWithPadding,
      ...
      emxEnsureCapacity_real_T(yRow, b_i, &ag_emlrtRTEI);
      for (b_i = 0; b_i < i; b_i++) {
      ...
      cpu_xIn->size[1] = 1;
      emxEnsureCapacity_real_T(cpu_xIn, b_i, &bg_emlrtRTEI);
      for (u1 = 0; u1 < i; u1++) {
      ...
      emxEnsureCapacity_real_T(cpu_xIn, b_i, &xf_emlrtRTEI);
      for (b_i = 0; b_i < i; b_i++) {
