...
static __global__ void upfirdn_kernel15(const emxArray_real_T x,
                                        const int32_T b_x,
...
static __global__ void upfirdn_kernel18(const emxArray_real_T w,
                                        const int32_T nElements_dim0,
                                        const int32_T nx, const int32_T b,
                                        const int32_T c, emxArray_real_T b_w,
                                        int32_T w_dim0);

static __global__ void upfirdn_kernel19(const emxArray_real_T w,
                                        const int32_T nElements,
                                        emxArray_real_T y_out);

static __global__ void upfirdn_kernel20(const emxArray_real_T w,
                                        const int32_T nElements_dim0,
                                        const int32_T nx, const int32_T b,
                                        const int32_T c, emxArray_real_T b_w,
                                        int32_T w_dim0);

static __global__ void upfirdn_kernel21(const emxArray_real_T w,
                                        const int32_T nElements_dim0,
                                        const int32_T b, const int32_T c,
                                        emxArray_real_T y_out,
                                        int32_T y_out_dim0);

static __global__ void upfirdn_kernel22(const emxArray_real_T y_out,
                                        const int32_T b,
                                        emxArray_real_T b_y_out,
                                        int32_T y_out_dim0);

static __global__ void upfirdn_kernel23(const emxArray_real_T y_out,
                                        const int32_T b_y_out,
                                        emxArray_real_T c_y_out);

static __global__ void upfirdn_kernel27(const emxArray_real_T x,
                                        const int32_T b_x,
                                        emxArray_real_T xCol);
             ...
             emxArray_real_T *gpu_y_out, boolean_T *y_out_outdatedOnGpu)
{
  ...
  emxInit_real_T(&cpu_xCol, 2, &gf_emlrtRTEI, true);
  if (xIsRow) {
    i = cpu_xCol->size[0] * cpu_xCol->size[1];
    cpu_xCol->size[0] = cpu_x->size[0] * cpu_x->size[1];
    cpu_xCol->size[1] = 1;
    emxEnsureCapacity_real_T(cpu_xCol, i, &gf_emlrtRTEI);
    ...
    emxEnsureCapacity_real_T(cpu_xCol, i, &ff_emlrtRTEI);
    i = cpu_x->size[0] * cpu_x->size[1] - 1;
    mwGetLaunchParameters1D(computeNumIters(i), &grid, &block, 2147483647U);
    gpuEmxEnsureCapacity_real_T(cpu_x, gpu_x, !*x_outdatedOnGpu);
    gpuEmxEnsureCapacity_real_T(cpu_xCol, &gpu_xCol, true);
  ...
  emxInit_real_T(&cpu_w, 2, &o_emlrtRTEI, true);
  emxInit_real_T(&A, 1, &o_emlrtRTEI, true);
  emxInit_real_T(&cpu_B, 1, &hf_emlrtRTEI, true);
  emxInit_real_T(&cpu_result, 1, &o_emlrtRTEI, true);
  b_yCol = *yCol;
  ...
  upfirdn_kernel17<<<dim3(1U, 1U, 1U), dim3(32U, 1U, 1U)>>>(nx, n, dim,
                                                            *gpu_origSiz);
  emxInit_real_T(&b_cpu_w, 2, &if_emlrtRTEI, true);
  i = b_cpu_w->size[0] * b_cpu_w->size[1];
  b_cpu_w->size[0] = div_s32(n, nx) + 1;
  b_cpu_w->size[1] = nElements;
  emxEnsureCapacity_real_T(b_cpu_w, i, &if_emlrtRTEI);
  i = n / nx;
  ...
  emxEnsureCapacity_real_T(cpu_y_out, i, &o_emlrtRTEI);
  ...
  if (!(cpu_origSiz[0] < Ly)) {
    ...
    emxInit_real_T(&c_cpu_w, 2, &if_emlrtRTEI, true);
    nElements = c_cpu_w->size[0] * c_cpu_w->size[1];
    c_cpu_w->size[0] = div_s32(n, nx) + 1;
    c_cpu_w->size[1] = cpu_lshift[1];
    emxEnsureCapacity_real_T(c_cpu_w, nElements, &if_emlrtRTEI);
    i = n / nx;
    ...
    cpu_y_out->size[1] = cpu_origSiz[1];
    emxEnsureCapacity_real_T(cpu_y_out, nElements, &p_emlrtRTEI);
  ...
  if (xIsRow) {
    nElements = cpu_y_out->size[0] * cpu_y_out->size[1];
    emxInit_real_T(&b_cpu_y_out, 2, &jf_emlrtRTEI, true);
    i = b_cpu_y_out->size[0] * b_cpu_y_out->size[1];
    b_cpu_y_out->size[0] = 1;
    b_cpu_y_out->size[1] = nElements;
    emxEnsureCapacity_real_T(b_cpu_y_out, i, &jf_emlrtRTEI);
    ...
    emxEnsureCapacity_real_T(cpu_y_out, i, &kf_emlrtRTEI);
