                                     ...
                                     ResampleParser *obj)
{
  ...
  emxEnsureCapacity_real_T(obj->x, inw, &gd_emlrtRTEI);
  ...
  b_dv[2] = 2.0 * fc;
  b_dv[3] = 1.0;
  emxInit_real_T(&r, 2, &hd_emlrtRTEI, true);
  firls(pqmax - 1.0, b_dv, r);
  emxInit_real_T(&c_dv, 1, &hd_emlrtRTEI, true);
  inw = c_dv->size[0];
  c_dv->size[0] = r->size[1];
  emxEnsureCapacity_real_T(c_dv, inw, &hd_emlrtRTEI);
  ...
  emxInit_real_T(&b_dv1, 1, &hd_emlrtRTEI, true);
  inw = b_dv1->size[0];
  b_dv1->size[0] = nw;
  emxEnsureCapacity_real_T(b_dv1, inw, &id_emlrtRTEI);
  if (nw <= 1) {
    inw = b_dv1->size[0];
    b_dv1->size[0] = nw;
  ...
  emxInit_real_T(&h1, 1, &kd_emlrtRTEI, true);
  inw = h1->size[0];
  h1->size[0] = c_dv->size[0];
  emxEnsureCapacity_real_T(h1, inw, &kd_emlrtRTEI);
  ...
  emxEnsureCapacity_real_T(obj->filter, inw, &ld_emlrtRTEI);
  ...
  emxEnsureCapacity_real_T(obj->filterWithPadding, inw, &md_emlrtRTEI);
  for (inw = 0;
  ...
  emxInit_uint32_T(&y, 2, &od_emlrtRTEI, true);
  ...
  emxInit_int32_T(&iv, 2, &pd_emlrtRTEI, true);
  inw = iv->size[0] * iv->size[1];
  iv->size[0] = 1;
  iv->size[1] = y->size[1];
  emxEnsureCapacity_int32_T(iv, inw, &nd_emlrtRTEI);
