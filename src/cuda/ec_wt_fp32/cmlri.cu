...
{
  real_T y;
  nvtxRangePushA("#fcn#rt_atan2d_snf#" MW_AT_LOCATION);
...
int32_T cmlri(const creal_T z, real_T fnu, int32_T kode, creal_T *y)
{
  ...
  boolean_T earlyExit;
  boolean_T exitg1;
  nvtxRangePushA("#fcn#cmlri#" MW_AT_LOCATION);
  nz = 0;
  ...
  i = 1;
  exitg1 = false;
  nvtxRangePushA("#loop#cmlri_whileloop_1##" MW_AT_LINE);
  while ((!exitg1) && (i < 81)) {
    icounter++;
    pt_re = p2_re;
    pt_im = p2_im;
  ...
  if (earlyExit) {
    ...
    if (static_cast<int32_T>(flooraz) <= 0) {
      ...
      i = 1;
      exitg1 = false;
      nvtxRangePushA("#loop#cmlri_whileloop_0##" MW_AT_LINE);
      while ((!exitg1) && (i < 81)) {
        kcounter++;
        pt_re = p2_re;
        pt_im = p2_im;
    ...
    if (guard1) {
      ...
      flooraz = 0.0;
      ck_re = 0.0;
      profileLoopStart("cmlri_loop_0", __LINE__, (kcounter - 1) + 1, "");
      ...
      if (rz_im == 0.0) {
        ...
        ack = std::log(rt_hypotd_snf(rz_re / 2.0, rz_im / 2.0)) +
              0.69314718055994529;
        nvtxMarkA("#rt_atan2d_snf#" MW_AT_LINE);
