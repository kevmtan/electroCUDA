...
static __global__ void upfirdn_kernel39(const emxArray_creal32_T x,
                                        const int32_T b_x,
                                        emxArray_creal32_T xCol);

static __global__ void upfirdn_kernel40(const int32_T nx, const int32_T b,
                                        int32_T lshift[2]);

static __global__ void upfirdn_kernel41(const int32_T nx, const int32_T n,
                                        const int32_T dim, int32_T origSiz[2]);

static __global__ void upfirdn_kernel42(const emxArray_creal32_T w,
                                        const int32_T nElements_dim0,
                                        const int32_T nx, const int32_T b,
                                        const int32_T c, emxArray_creal32_T b_w,
                                        int32_T w_dim0);

static __global__ void upfirdn_kernel43(const emxArray_creal32_T w,
                                        const int32_T nElements,
                                        emxArray_creal32_T y_out);

static __global__ void upfirdn_kernel44(const emxArray_creal32_T w,
                                        const int32_T nElements_dim0,
                                        const int32_T nx, const int32_T b,
                                        const int32_T c, emxArray_creal32_T b_w,
                                        int32_T w_dim0);

static __global__ void upfirdn_kernel45(const emxArray_creal32_T w,
                                        const int32_T nElements_dim0,
                                        const int32_T b, const int32_T c,
                                        emxArray_creal32_T y_out,
                                        int32_T y_out_dim0);

static __global__ void upfirdn_kernel46(const emxArray_creal32_T y_out,
                                        const int32_T b,
                                        emxArray_creal32_T b_y_out,
                                        int32_T y_out_dim0);

static __global__ void upfirdn_kernel47(const emxArray_creal32_T y_out,
                                        const int32_T b_y_out,
                                        emxArray_creal32_T c_y_out);

static __global__ void upfirdn_kernel51(const emxArray_creal32_T x,
                                        const int32_T b_x,
                                        emxArray_creal32_T xCol);
...
static __global__ __launch_bounds__(1024,
                                    1) void upfirdn_kernel40(const int32_T nx,
                                                             const int32_T b,
                                                             int32_T lshift[2])
{
  uint64_T gStride;
  ...
  for (uint64_T idx{gThreadId}; idx <= loopEnd; idx += gStride) {
    int32_T i5;
    i5 = lshift[0];
    lshift[0] = lshift[1 - nx];
    lshift[1 - nx] = i5;
  }
}

static __global__ __launch_bounds__(32, 1) void upfirdn_kernel41(
    const int32_T nx, const int32_T n, const int32_T dim, int32_T origSiz[2])
{
  int32_T tmpIdx;
  tmpIdx = static_cast<int32_T>(mwGetGlobalThreadIndex());
  if (tmpIdx < 1) {
    origSiz[dim - 1] = div_s32_device(n, nx) + 1;
  }
}
    ...
    const int32_T b, const int32_T c, emxArray_creal32_T b_w, int32_T w_dim0)
{
  ...
  for (uint64_T idx{gThreadId}; idx <= loopEnd; idx += gStride) {
    int32_T i1;
    int32_T i5;
    i5 = static_cast<int32_T>(idx % (static_cast<uint64_T>(b) + 1UL));
    i1 = static_cast<int32_T>((idx - static_cast<uint64_T>(i5)) /
                              (static_cast<uint64_T>(b) + 1UL));
    b_w.data[i5 + w_dim0 * i1] = w.data[nx * i5 + nElements_dim0 * i1];
  }
}

static __global__
    __launch_bounds__(1024, 1) void upfirdn_kernel43(const emxArray_creal32_T w,
                                                     const int32_T nElements,
                                                     emxArray_creal32_T y_out)
{
  uint64_T gStride;
...
static __global__ __launch_bounds__(1024, 1) void upfirdn_kernel44(
    const emxArray_creal32_T w, const int32_T nElements_dim0, const int32_T nx,
    const int32_T b, const int32_T c, emxArray_creal32_T b_w, int32_T w_dim0)
{
  uint64_T gStride;
...
static __global__ __launch_bounds__(1024, 1) void upfirdn_kernel45(
    ...
    const int32_T c, emxArray_creal32_T y_out, int32_T y_out_dim0)
{
  ...
  for (uint64_T idx{gThreadId}; idx <= loopEnd; idx += gStride) {
    ...
    i1 = static_cast<int32_T>((idx - static_cast<uint64_T>(i5)) /
                              (static_cast<uint64_T>(b) + 1UL));
    y_out.data[i5 + y_out_dim0 * i1] = w.data[i5 + nElements_dim0 * i1];
  }
}

static __global__ __launch_bounds__(1024, 1) void upfirdn_kernel46(
...
static __global__ __launch_bounds__(1024, 1) void upfirdn_kernel47(
...
static __global__ __launch_bounds__(1024, 1) void upfirdn_kernel51(
    const emxArray_creal32_T x, const int32_T b_x, emxArray_creal32_T xCol)
{
  uint64_T gStride;
  uint64_T gThreadId;
...
//
namespace coder {
void upfirdn(emxArray_creal32_T *cpu_x, emxArray_creal32_T *gpu_x,
             ...
             emxArray_creal32_T *gpu_y_out, boolean_T *y_out_outdatedOnGpu)
{
  ...
  boolean_T result_outdatedOnGpu;
  ...
  if (xIsRow) {
    ...
    emxEnsureCapacity_creal32_T(cpu_xCol, i, &sg_emlrtRTEI);
    ...
    if (validLaunchParams) {
      nvtxMarkA("#upfirdn_kernel39#" MW_AT_LINE);
      upfirdn_kernel39<<<grid, block>>>(*gpu_x, i, gpu_xCol);
    }
    ...
    nvtxMarkA("#emxEnsureCapacity_creal32_T#" MW_AT_LINE);
    emxEnsureCapacity_creal32_T(cpu_xCol, i, &rg_emlrtRTEI);
    i = cpu_x->size[0] * cpu_x->size[1] - 1;
    nvtxMarkA("#computeNumIters#" MW_AT_LINE);
    mwGetLaunchParameters1D(computeNumIters(i), &grid, &block, 2147483647U);
    nvtxMarkA("#gpuEmxEnsureCapacity_creal32_T#" MW_AT_LINE);
    ...
    validLaunchParams = mwValidateLaunchParameters(grid, block);
    if (validLaunchParams) {
      nvtxMarkA("#upfirdn_kernel51#" MW_AT_LINE);
      upfirdn_kernel51<<<grid, block>>>(*gpu_x, i, gpu_xCol);
    }
    result_outdatedOnCpu = true;
  }
  if (!(varargin_1 - 1.0 >= 0.0)) {
    ...
    emlrt_marshallIn(b_sprintf(y, b_y, &f_emlrtMCI), "<output of sprintf>",
                     unusedExpr);
  }
  ...
  nvtxMarkA("#emxInit_creal32_T#" MW_AT_LINE);
  emxInit_creal32_T(&yCol, 1, &qe_emlrtRTEI, true);
  i = yCol->size[0];
  yCol->size[0] = nx * static_cast<int32_T>(varargin_1);
  nvtxMarkA("#emxEnsureCapacity_creal32_T#" MW_AT_LINE);
  emxEnsureCapacity_creal32_T(yCol, i, &ne_emlrtRTEI);
  ...
  nvtxMarkA("#emxInit_creal32_T#" MW_AT_LINE);
  emxInit_creal32_T(&cpu_w, 2, &oe_emlrtRTEI, true);
  nvtxMarkA("#emxInit_creal32_T#" MW_AT_LINE);
  emxInit_creal32_T(&A, 1, &oe_emlrtRTEI, true);
  nvtxMarkA("#emxInit_real32_T#" MW_AT_LINE);
  emxInit_real32_T(&cpu_B, 1, &tg_emlrtRTEI, true);
  nvtxMarkA("#emxInit_creal32_T#" MW_AT_LINE);
  emxInit_creal32_T(&cpu_result, 1, &oe_emlrtRTEI, true);
  b_yCol = *yCol;
  ...
  conv2(&b_yCol, cpu_h, h_outdatedOnCpu, gpu_h, h_outdatedOnGpu, cpu_w,
        &result_outdatedOnGpu, &gpu_w, &w_outdatedOnGpu);
  ...
  if (!(varargin_2 - 1.0 >= 0.0)) {
    ...
    nvtxMarkA("#emlrt_marshallIn#" MW_AT_LINE);
    nvtxMarkA("#b_sprintf#" MW_AT_LINE);
    emlrt_marshallIn(b_sprintf(c_y, d_y, &f_emlrtMCI), "<output of sprintf>",
                     unusedExpr);
  }
  dim = 2;
  if (cpu_w->size[0] != 1) {
  ...
  if (1 - dim >= 0) {
    ...
    validLaunchParams = mwValidateLaunchParameters(grid, block);
    if (validLaunchParams) {
      nvtxMarkA("#upfirdn_kernel40#" MW_AT_LINE);
      upfirdn_kernel40<<<grid, block>>>(
          1 - dim,
          static_cast<int32_T>((2.0 - (1.0 - static_cast<real_T>(dim))) / 2.0) -
              1,
          *gpu_lshift);
    }
    nvtxMarkA("#checkCudaError#" MW_AT_LINE);
  ...
  nElements = cpu_lshift[0];
  cpu_lshift[0] = cpu_lshift[1];
  cpu_lshift[1] = nElements;
  ...
  nvtxMarkA("#upfirdn_kernel41#" MW_AT_LINE);
  upfirdn_kernel41<<<dim3(1U, 1U, 1U), dim3(32U, 1U, 1U)>>>(nx, n, dim,
                                                            *gpu_origSiz);
  nvtxMarkA("#emxInit_creal32_T#" MW_AT_LINE);
  emxInit_creal32_T(&b_cpu_w, 2, &ug_emlrtRTEI, true);
  i = b_cpu_w->size[0] * b_cpu_w->size[1];
  nvtxMarkA("#div_s32#" MW_AT_LINE);
  b_cpu_w->size[0] = div_s32(n, nx) + 1;
  b_cpu_w->size[1] = nElements;
  nvtxMarkA("#emxEnsureCapacity_creal32_T#" MW_AT_LINE);
  emxEnsureCapacity_creal32_T(b_cpu_w, i, &ug_emlrtRTEI);
  i = n / nx;
  nvtxMarkA("#computeNumIters#" MW_AT_LINE);
  mwGetLaunchParameters1D(computeNumIters(nElements - 1, i), &grid, &block,
                          2147483647U);
  ...
  if (validLaunchParams) {
    ...
    upfirdn_kernel42<<<grid, block>>>(
        gpu_w, cpu_lshift[0], nx, i, nElements - 1, b_gpu_w, b_cpu_w->size[0U]);
  }
  nvtxMarkA("#checkCudaError#" MW_AT_LINE);
  nvtxMarkA("#cudaMemcpy#" MW_AT_LINE);
  checkCudaError(
  ...
  nvtxMarkA("#emxEnsureCapacity_creal32_T#" MW_AT_LINE);
  emxEnsureCapacity_creal32_T(cpu_y_out, i, &oe_emlrtRTEI);
  ...
  validLaunchParams = mwValidateLaunchParameters(grid, block);
  if (validLaunchParams) {
    nvtxMarkA("#upfirdn_kernel43#" MW_AT_LINE);
    upfirdn_kernel43<<<grid, block>>>(b_gpu_w, nElements, *gpu_y_out);
  }
  nvtxMarkA("#emxFree_creal32_T#" MW_AT_LINE);
  ...
  if (!(cpu_origSiz[0] < Ly)) {
    ...
    nvtxMarkA("#emxInit_creal32_T#" MW_AT_LINE);
    emxInit_creal32_T(&c_cpu_w, 2, &ug_emlrtRTEI, true);
    nElements = c_cpu_w->size[0] * c_cpu_w->size[1];
    nvtxMarkA("#div_s32#" MW_AT_LINE);
    c_cpu_w->size[0] = div_s32(n, nx) + 1;
    c_cpu_w->size[1] = cpu_lshift[1];
    nvtxMarkA("#emxEnsureCapacity_creal32_T#" MW_AT_LINE);
    emxEnsureCapacity_creal32_T(c_cpu_w, nElements, &ug_emlrtRTEI);
    i = n / nx;
    nvtxMarkA("#computeNumIters#" MW_AT_LINE);
    ...
    validLaunchParams = mwValidateLaunchParameters(grid, block);
    if (validLaunchParams) {
      nvtxMarkA("#upfirdn_kernel44#" MW_AT_LINE);
      upfirdn_kernel44<<<grid, block>>>(gpu_w, cpu_lshift[0], nx, i,
    ...
    cpu_y_out->size[1] = cpu_origSiz[1];
    nvtxMarkA("#emxEnsureCapacity_creal32_T#" MW_AT_LINE);
    emxEnsureCapacity_creal32_T(cpu_y_out, nElements, &pe_emlrtRTEI);
    nvtxMarkA("#computeNumIters#" MW_AT_LINE);
    ...
    validLaunchParams = mwValidateLaunchParameters(grid, block);
    if (validLaunchParams) {
      nvtxMarkA("#upfirdn_kernel45#" MW_AT_LINE);
      upfirdn_kernel45<<<grid, block>>>(c_gpu_w, cpu_origSiz[0], b_i - 1,
  ...
  if (xIsRow) {
    nElements = cpu_y_out->size[0] * cpu_y_out->size[1];
    nvtxMarkA("#emxInit_creal32_T#" MW_AT_LINE);
    emxInit_creal32_T(&b_cpu_y_out, 2, &vg_emlrtRTEI, true);
    i = b_cpu_y_out->size[0] * b_cpu_y_out->size[1];
    b_cpu_y_out->size[0] = 1;
    b_cpu_y_out->size[1] = nElements;
    nvtxMarkA("#emxEnsureCapacity_creal32_T#" MW_AT_LINE);
    emxEnsureCapacity_creal32_T(b_cpu_y_out, i, &vg_emlrtRTEI);
    nvtxMarkA("#computeNumIters#" MW_AT_LINE);
    ...
    validLaunchParams = mwValidateLaunchParameters(grid, block);
    if (validLaunchParams) {
      nvtxMarkA("#upfirdn_kernel46#" MW_AT_LINE);
      upfirdn_kernel46<<<grid, block>>>(*gpu_y_out, nElements - 1, b_gpu_y_out,
                                        b_cpu_y_out->size[0U]);
    }
    i = cpu_y_out->size[0] * cpu_y_out->size[1];
    cpu_y_out->size[0] = 1;
    ...
    nvtxMarkA("#emxEnsureCapacity_creal32_T#" MW_AT_LINE);
    emxEnsureCapacity_creal32_T(cpu_y_out, i, &wg_emlrtRTEI);
    ...
    validLaunchParams = mwValidateLaunchParameters(grid, block);
    if (validLaunchParams) {
      nvtxMarkA("#upfirdn_kernel47#" MW_AT_LINE);
      upfirdn_kernel47<<<grid, block>>>(b_gpu_y_out, i, *gpu_y_out);
    }
    nvtxMarkA("#emxFree_creal32_T#" MW_AT_LINE);
