                                     ...
                                     ResampleParser *obj)
{
  ...
  emxEnsureCapacity_real32_T(obj->x, inw, &hd_emlrtRTEI);
  ...
  b_dv[2] = 2.0 * fc;
  b_dv[3] = 1.0;
  emxInit_real_T(&r, 2, &id_emlrtRTEI, true);
  firls(pqmax - 1.0, b_dv, r);
  emxInit_real_T(&c_dv, 1, &id_emlrtRTEI, true);
  inw = c_dv->size[0];
  c_dv->size[0] = r->size[1];
  emxEnsureCapacity_real_T(c_dv, inw, &id_emlrtRTEI);
  ...
  emxInit_real_T(&b_dv1, 1, &id_emlrtRTEI, true);
  inw = b_dv1->size[0];
  b_dv1->size[0] = nw;
  emxEnsureCapacity_real_T(b_dv1, inw, &jd_emlrtRTEI);
  if (nw <= 1) {
    inw = b_dv1->size[0];
    b_dv1->size[0] = nw;
    emxEnsureCapacity_real_T(b_dv1, inw, &kd_emlrtRTEI);
  ...
  emxInit_real_T(&h1, 1, &ld_emlrtRTEI, true);
  inw = h1->size[0];
  h1->size[0] = c_dv->size[0];
  emxEnsureCapacity_real_T(h1, inw, &ld_emlrtRTEI);
  ...
  emxEnsureCapacity_real32_T(obj->filter, inw, &md_emlrtRTEI);
  for (inw = 0; inw < h1->size[0]; inw++) {
    obj->filter->data[inw] =
        static_cast<real32_T>(obj_p * h1->data[inw] / pqmax);
  }
  emxFree_real_T(&h1);
  ...
  emxEnsureCapacity_real32_T(obj->filterWithPadding, inw, &nd_emlrtRTEI);
  for (inw = 0;
       inw < static_cast<int32_T>(
                 (ak + static_cast<real_T>(obj->filter->size[0])) + fc);
       inw++) {
    obj->filterWithPadding->data[inw] = 0.0F;
  }
  emxInit_uint32_T(&y, 2, &pd_emlrtRTEI, true);
  if (obj->filter->size[0] < 1) {
    ...
    emxEnsureCapacity_uint32_T(y, inw, &q_emlrtRTEI);
    for (inw = 0; inw < static_cast<int32_T>(u); inw++) {
      y->data[inw] = static_cast<uint32_T>(inw) + 1U;
    }
  }
  emxInit_int32_T(&iv, 2, &qd_emlrtRTEI, true);
  inw = iv->size[0] * iv->size[1];
  iv->size[0] = 1;
  iv->size[1] = y->size[1];
  emxEnsureCapacity_int32_T(iv, inw, &od_emlrtRTEI);
