...
static __global__ void upfirdn_kernel15(const emxArray_real32_T x,
                                        const int32_T b_x,
                                        emxArray_real32_T xCol);

static __global__ void upfirdn_kernel16(const int32_T nx, const int32_T b,
                                        int32_T lshift[2]);

static __global__ void upfirdn_kernel17(const int32_T nx, const int32_T n,
                                        const int32_T dim, int32_T origSiz[2]);

static __global__ void upfirdn_kernel18(const emxArray_real32_T w,
                                        const int32_T nElements_dim0,
                                        const int32_T nx, const int32_T b,
                                        const int32_T c, emxArray_real32_T b_w,
                                        int32_T w_dim0);

static __global__ void upfirdn_kernel19(const emxArray_real32_T w,
                                        const int32_T nElements,
                                        emxArray_real32_T y_out);

static __global__ void upfirdn_kernel20(const emxArray_real32_T w,
                                        const int32_T nElements_dim0,
                                        const int32_T nx, const int32_T b,
                                        const int32_T c, emxArray_real32_T b_w,
                                        int32_T w_dim0);

static __global__ void upfirdn_kernel21(const emxArray_real32_T w,
                                        const int32_T nElements_dim0,
                                        const int32_T b, const int32_T c,
                                        emxArray_real32_T y_out,
                                        int32_T y_out_dim0);

static __global__ void upfirdn_kernel22(const emxArray_real32_T y_out,
                                        const int32_T b,
                                        emxArray_real32_T b_y_out,
                                        int32_T y_out_dim0);

static __global__ void upfirdn_kernel23(const emxArray_real32_T y_out,
                                        const int32_T b_y_out,
                                        emxArray_real32_T c_y_out);

static __global__ void upfirdn_kernel27(const emxArray_real32_T x,
                                        const int32_T b_x,
                                        emxArray_real32_T xCol);
...
static __global__ __launch_bounds__(1024, 1) void upfirdn_kernel15(
...
static __global__ __launch_bounds__(1024,
                                    1) void upfirdn_kernel16(const int32_T nx,
                                                             const int32_T b,
                                                             int32_T lshift[2])
{
  uint64_T gStride;
...
static __global__ __launch_bounds__(32, 1) void upfirdn_kernel17(
...
static __global__ __launch_bounds__(1024, 1) void upfirdn_kernel18(
...
static __global__ __launch_bounds__(1024, 1) void upfirdn_kernel19(
...
static __global__ __launch_bounds__(1024, 1) void upfirdn_kernel20(
    const emxArray_real32_T w, const int32_T nElements_dim0, const int32_T nx,
    const int32_T b, const int32_T c, emxArray_real32_T b_w, int32_T w_dim0)
{
  uint64_T gStride;
...
static __global__ __launch_bounds__(1024, 1) void upfirdn_kernel21(
...
static __global__ __launch_bounds__(1024, 1) void upfirdn_kernel22(
...
static __global__ __launch_bounds__(1024, 1) void upfirdn_kernel23(
...
static __global__ __launch_bounds__(1024, 1) void upfirdn_kernel27(
    const emxArray_real32_T x, const int32_T b_x, emxArray_real32_T xCol)
{
  uint64_T gStride;
  uint64_T gThreadId;
             ...
             emxArray_real32_T *gpu_y_out, boolean_T *y_out_outdatedOnGpu)
{
  ...
  emxInit_real32_T(&cpu_xCol, 2, &hf_emlrtRTEI, true);
  if (xIsRow) {
    i = cpu_xCol->size[0] * cpu_xCol->size[1];
    cpu_xCol->size[0] = cpu_x->size[0] * cpu_x->size[1];
    cpu_xCol->size[1] = 1;
    emxEnsureCapacity_real32_T(cpu_xCol, i, &hf_emlrtRTEI);
    ...
    if (validLaunchParams) {
      upfirdn_kernel15<<<grid, block>>>(*gpu_x, i, gpu_xCol);
    }
    ...
    emxEnsureCapacity_real32_T(cpu_xCol, i, &gf_emlrtRTEI);
    i = cpu_x->size[0] * cpu_x->size[1] - 1;
    mwGetLaunchParameters1D(computeNumIters(i), &grid, &block, 2147483647U);
    gpuEmxEnsureCapacity_real32_T(cpu_x, gpu_x, !*x_outdatedOnGpu);
    gpuEmxEnsureCapacity_real32_T(cpu_xCol, &gpu_xCol, true);
    ...
    validLaunchParams = mwValidateLaunchParameters(grid, block);
    if (validLaunchParams) {
      upfirdn_kernel27<<<grid, block>>>(*gpu_x, i, gpu_xCol);
    }
    xCol_outdatedOnCpu = true;
  }
  ...
  emxInit_real32_T(&yCol, 1, &v_emlrtRTEI, true);
  i = yCol->size[0];
  yCol->size[0] = nx * static_cast<int32_T>(varargin_1);
  emxEnsureCapacity_real32_T(yCol, i, &n_emlrtRTEI);
  ...
  emxInit_real32_T(&cpu_w, 2, &o_emlrtRTEI, true);
  emxInit_real32_T(&A, 1, &o_emlrtRTEI, true);
  emxInit_real32_T(&cpu_B, 1, &if_emlrtRTEI, true);
  emxInit_real32_T(&cpu_result, 1, &o_emlrtRTEI, true);
  b_yCol = *yCol;
  ...
  if (!(varargin_2 - 1.0 >= 0.0)) {
    ...
    emlrtAssign(&d_y, m3);
    emlrt_marshallIn(b_sprintf(c_y, d_y, &e_emlrtMCI), "<output of sprintf>",
                     unusedExpr);
  }
  dim = 2;
  if (cpu_w->size[0] != 1) {
  ...
  upfirdn_kernel17<<<dim3(1U, 1U, 1U), dim3(32U, 1U, 1U)>>>(nx, n, dim,
                                                            *gpu_origSiz);
  emxInit_real32_T(&b_cpu_w, 2, &jf_emlrtRTEI, true);
  i = b_cpu_w->size[0] * b_cpu_w->size[1];
  b_cpu_w->size[0] = div_s32(n, nx) + 1;
  b_cpu_w->size[1] = nElements;
  emxEnsureCapacity_real32_T(b_cpu_w, i, &jf_emlrtRTEI);
  i = n / nx;
  ...
  emxEnsureCapacity_real32_T(cpu_y_out, i, &o_emlrtRTEI);
  ...
  validLaunchParams = mwValidateLaunchParameters(grid, block);
  if (validLaunchParams) {
    upfirdn_kernel19<<<grid, block>>>(b_gpu_w, nElements, *gpu_y_out);
  }
  ...
  if (!(cpu_origSiz[0] < Ly)) {
    ...
    emxInit_real32_T(&c_cpu_w, 2, &jf_emlrtRTEI, true);
    nElements = c_cpu_w->size[0] * c_cpu_w->size[1];
    c_cpu_w->size[0] = div_s32(n, nx) + 1;
    c_cpu_w->size[1] = cpu_lshift[1];
    emxEnsureCapacity_real32_T(c_cpu_w, nElements, &jf_emlrtRTEI);
    i = n / nx;
    ...
    validLaunchParams = mwValidateLaunchParameters(grid, block);
    if (validLaunchParams) {
      upfirdn_kernel20<<<grid, block>>>(gpu_w, cpu_lshift[0], nx, i,
    ...
    cpu_y_out->size[1] = cpu_origSiz[1];
    emxEnsureCapacity_real32_T(cpu_y_out, nElements, &p_emlrtRTEI);
    mwGetLaunchParameters1D(computeNumIters(cpu_origSiz[1] - 1, b_i - 1), &grid,
                            &block, 2147483647U);
    gpuEmxEnsureCapacity_real32_T(cpu_y_out, gpu_y_out, true);
    validLaunchParams = mwValidateLaunchParameters(grid, block);
    if (validLaunchParams) {
      upfirdn_kernel21<<<grid, block>>>(c_gpu_w, cpu_origSiz[0], b_i - 1,
  ...
  if (xIsRow) {
    nElements = cpu_y_out->size[0] * cpu_y_out->size[1];
    emxInit_real32_T(&b_cpu_y_out, 2, &kf_emlrtRTEI, true);
    i = b_cpu_y_out->size[0] * b_cpu_y_out->size[1];
    b_cpu_y_out->size[0] = 1;
    b_cpu_y_out->size[1] = nElements;
    emxEnsureCapacity_real32_T(b_cpu_y_out, i, &kf_emlrtRTEI);
    mwGetLaunchParameters1D(computeNumIters(nElements - 1), &grid, &block,
                            2147483647U);
    gpuEmxEnsureCapacity_real32_T(b_cpu_y_out, &b_gpu_y_out, true);
    validLaunchParams = mwValidateLaunchParameters(grid, block);
    if (validLaunchParams) {
      upfirdn_kernel22<<<grid, block>>>(*gpu_y_out, nElements - 1, b_gpu_y_out,
                                        b_cpu_y_out->size[0U]);
    }
    i = cpu_y_out->size[0] * cpu_y_out->size[1];
    cpu_y_out->size[0] = 1;
    cpu_y_out->size[1] = b_cpu_y_out->size[1];
    emxEnsureCapacity_real32_T(cpu_y_out, i, &lf_emlrtRTEI);
    i = b_cpu_y_out->size[1] - 1;
    mwGetLaunchParameters1D(computeNumIters(i), &grid, &block, 2147483647U);
    gpuEmxEnsureCapacity_real32_T(cpu_y_out, gpu_y_out, true);
    validLaunchParams = mwValidateLaunchParameters(grid, block);
    if (validLaunchParams) {
      upfirdn_kernel23<<<grid, block>>>(b_gpu_y_out, i, *gpu_y_out);
    }
