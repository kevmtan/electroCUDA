...
static __global__ void ec_cwt_kernel1(const int32_T ch,
                                      const emxArray_real32_T x,
                                      const int32_T b_x, emxArray_real32_T c_x,
                                      int32_T x_dim0);

static __global__ void ec_cwt_kernel10(const emxArray_real32_T xCh,
                                       const int32_T b_xCh, const int32_T c_xCh,
                                       emxArray_real32_T opts_x,
                                       int32_T opts_x_dim0, int32_T xCh_dim0);

static __global__ void ec_cwt_kernel2(const emxArray_creal32_T x,
                                      const int32_T b, emxArray_real32_T y);

static __global__ void ec_cwt_kernel3(const emxArray_real32_T y,
                                      const int32_T b_y, const int32_T c_y,
                                      emxArray_real32_T xCh, int32_T xCh_dim0,
                                      int32_T y_dim0);

static __global__ void ec_cwt_kernel4(const emxArray_real32_T xCh,
                                      const int32_T b,
                                      emxArray_real32_T opts_x);

static __global__ void ec_cwt_kernel5(const emxArray_real32_T w,
                                      const int32_T nx, const int32_T b,
                                      emxArray_real32_T y);

static __global__ void ec_cwt_kernel6(emxArray_real_T y);

static __global__ void ec_cwt_kernel7(const emxArray_real32_T y,
                                      const emxArray_real_T b_y,
                                      const real_T nlast, const int32_T ii,
                                      const int32_T b, emxArray_real32_T b_y1,
                                      int32_T y1_dim0);

static __global__ void ec_cwt_kernel8(const int32_T b, emxArray_real_T y);

static __global__ void ec_cwt_kernel9(const emxArray_real32_T xCh,
                                      const int32_T b_xCh,
                                      emxArray_real32_T opts_x);
...
static __global__ __launch_bounds__(1024, 1) void ec_cwt_kernel10(
    const emxArray_real32_T xCh, const int32_T b_xCh, const int32_T c_xCh,
    emxArray_real32_T opts_x, int32_T opts_x_dim0, int32_T xCh_dim0)
{
  uint64_T gStride;
  uint64_T gThreadId;
  uint64_T loopEnd;
...
static __global__ __launch_bounds__(1024, 1) void ec_cwt_kernel3(
    const emxArray_real32_T y, const int32_T b_y, const int32_T c_y,
    emxArray_real32_T xCh, int32_T xCh_dim0, int32_T y_dim0)
{
  uint64_T gStride;
  uint64_T gThreadId;
  uint64_T loopEnd;
...
static __global__ __launch_bounds__(1024, 1) void ec_cwt_kernel4(
    const emxArray_real32_T xCh, const int32_T b, emxArray_real32_T opts_x)
{
  uint64_T gStride;
  uint64_T gThreadId;
  uint64_T loopEnd;
...
static __global__ __launch_bounds__(1024, 1) void ec_cwt_kernel5(
    const emxArray_real32_T w, const int32_T nx, const int32_T b,
    emxArray_real32_T y)
{
  uint64_T gStride;
  uint64_T gThreadId;
...
static __global__ __launch_bounds__(1024, 1) void ec_cwt_kernel7(
    const emxArray_real32_T y, const emxArray_real_T b_y, const real_T nlast,
    const int32_T ii, const int32_T b, emxArray_real32_T b_y1, int32_T y1_dim0)
{
  uint64_T gStride;
  uint64_T gThreadId;
  uint64_T loopEnd;
...
static __global__ __launch_bounds__(1024, 1) void ec_cwt_kernel9(
    const emxArray_real32_T xCh, const int32_T b_xCh, emxArray_real32_T opts_x)
{
  uint64_T gStride;
  uint64_T gThreadId;
  uint64_T loopEnd;
            ...
            emxArray_real_T *freqs)
{
  ...
  i1 = cpu_x->size[1];
  emxInit_real32_T(&cpu_y, 2, &s_emlrtRTEI, true);
  emxInit_creal32_T(&b_cpu_x, 2, &t_emlrtRTEI, true);
  emxInit_real32_T(&cpu_xCh, 2, &e_emlrtRTEI, true);
  emxInit_real32_T(&cpu_opts_x, 2, &g_emlrtRTEI, true);
  emxInit_real32_T(&cpu_y1, 2, &u_emlrtRTEI, true);
  emxInit_real32_T(&yCol, 1, &v_emlrtRTEI, true);
  emxInit_real32_T(&cpu_w, 1, &o_emlrtRTEI, true);
  emxInit_real32_T(&b_cpu_y, 1, &o_emlrtRTEI, true);
  emxInit_real_T(&c_cpu_y, 2, &w_emlrtRTEI, true);
  emxInitStruct_ResampleParser(&expl_temp, &g_emlrtRTEI, true);
  emxInit_real32_T(&c_cpu_x, 1, &c_emlrtRTEI, true);
  for (int32_T ch{0}; ch < i1; ch++) {
    ...
    emxEnsureCapacity_real32_T(c_cpu_x, i, &c_emlrtRTEI);
    i = cpu_x->size[0] - 1;
    ...
    mwGetLaunchParameters1D(computeNumIters(nx - 1), &grid, &block,
                            2147483647U);
    gpuEmxEnsureCapacity_creal32_T(b_cpu_x, &c_gpu_x, false);
    gpuEmxEnsureCapacity_real32_T(cpu_y, &gpu_y, true);
    gpuEmxMemcpyCpuToGpu_creal32_T(&c_gpu_x, b_cpu_x);
    validLaunchParams = mwValidateLaunchParameters(grid, block);
    if (validLaunchParams) {
    ...
    emxEnsureCapacity_real32_T(cpu_xCh, i, &e_emlrtRTEI);
    i = cpu_y->size[0] - 1;
    f_y = cpu_y->size[1] - 1;
    ...
    if (doDownsample) {
      ...
      if (expl_temp.dim == 1) {
        if (expl_temp.isRowVectorInput) {
          ...
          emxEnsureCapacity_real32_T(cpu_y1, i, &i_emlrtRTEI);
          ...
          emxEnsureCapacity_real32_T(expl_temp.x, i, &m_emlrtRTEI);
          for (i = 0; i < f_y; i++) {
        ...
        if (expl_temp.x->size[0] == 1) {
          ...
          cpu_y1->size[1] = expl_temp.x->size[1];
          emxEnsureCapacity_real32_T(cpu_y1, i, &k_emlrtRTEI);
          ...
          for (int32_T ii{0}; ii < opts_dim; ii++) {
            ...
            if (!(div_s32(f_y, nx) + 1 < Ly)) {
              ...
              emxEnsureCapacity_real32_T(b_cpu_y, i, &p_emlrtRTEI);
              xCh_outdatedOnCpu = true;
            }
            if (std::isnan(dlast)) {
          ...
          cpu_y1->size[0] = expl_temp.x->size[0];
          cpu_y1->size[1] = expl_temp.x->size[1];
          emxEnsureCapacity_real32_T(cpu_y1, i, &h_emlrtRTEI);
          for (i = 0; i < expl_temp.x->size[0] * expl_temp.x->size[1]; i++) {
            cpu_y1->data[i] = expl_temp.x->data[i];
          }
          coder::b_signal::internal::resample::uniformResampleAlongFirstDim(
        ...
        if ((expl_temp.x->size[0] == 1) || (expl_temp.x->size[1] == 1)) {
          ...
          emxEnsureCapacity_real32_T(expl_temp.x, i, &l_emlrtRTEI);
          for (i = 0; i < nx * f_y; i++) {
            if (xCh_outdatedOnCpu) {
              gpuEmxMemcpyGpuToCpu_real32_T(cpu_y1, &gpu_y1);
            }
            xCh_outdatedOnCpu = false;
            expl_temp.x->data[i] = cpu_y1->data[i];
          }
      ...
      emxEnsureCapacity_real32_T(y->data[ch].f1, i, &j_emlrtRTEI);
      i = y->data[ch].f1->size[0] * y->data[ch].f1->size[1];
      ...
      emxEnsureCapacity_real32_T(y->data[ch].f1, i, &f_emlrtRTEI);
      i = y->data[ch].f1->size[0] * y->data[ch].f1->size[1];
