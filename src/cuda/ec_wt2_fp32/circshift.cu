...
static __global__ void circshift_kernel16(const int32_T shiftLen,
                                          const int32_T b, int32_T a[2]);

// Function Definitions
static __global__ __launch_bounds__(1024, 1) void circshift_kernel16(
               ...
               boolean_T *a_outdatedOnGpu, int32_T p)
{
  ...
  if (shiftLen < 2 - shiftLen) {
    ...
    if (validLaunchParams) {
      nvtxMarkA("#circshift_kernel16#" MW_AT_LINE);
      circshift_kernel16<<<grid, block>>>(
          shiftLen,
          static_cast<int32_T>(static_cast<real_T>(2 - shiftLen) / 2.0) - 1,
          gpu_a);
    }
