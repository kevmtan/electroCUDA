                        ...
                        const emxArray_real_T *in3, const emxArray_real_T *in4)
{
  ...
  emxEnsureCapacity_real_T(in1, i, &oc_emlrtRTEI);
...
real_T quadgk(real_T fun_workspace_be, real_T fun_workspace_ga)
{
  ...
  if (!(pathlen > 0.0)) {
    ...
    emxInit_real_T(&b_x, 2, &pc_emlrtRTEI, true);
    emxInit_real_T(&xt, 2, &qc_emlrtRTEI, true);
    emxInit_real_T(&y, 2, &wc_emlrtRTEI, true);
    emxInit_real_T(&fx, 2, &xc_emlrtRTEI, true);
    emxInit_real_T(&b_dv1, 2, &yc_emlrtRTEI, true);
    emxInit_real_T(&b_y, 2, &ad_emlrtRTEI, true);
    emxInit_real_T(&c_y, 2, &bd_emlrtRTEI, true);
    emxInit_real_T(&d_y, 2, &yc_emlrtRTEI, true);
    emxInit_real_T(&a, 2, &vc_emlrtRTEI, true);
    int32_T exitg1;
    do {
      real_T d;
      real_T halfh;
      ...
      x->size[0] = 1;
      x->size[1] = 15 * (nsubs + 1);
      emxEnsureCapacity_real_T(x, ix, &jc_emlrtRTEI);
      ix = -1;
      for (k = 0; k <= nsubs; k++) {
        d = subs[k << 1];
        tau = subs[(k << 1) + 1];
      ...
      b_x->size[0] = 1;
      b_x->size[1] = x->size[1];
      emxEnsureCapacity_real_T(b_x, ix, &kc_emlrtRTEI);
      ix = xt->size[0] * xt->size[1];
      xt->size[0] = 1;
      xt->size[1] = x->size[1];
      emxEnsureCapacity_real_T(xt, ix, &lc_emlrtRTEI);
      ix = x->size[1];
      for (k = 0; k < ix; k++) {
        d = x->data[k];
        midpt = d / (1.0 - d);
      ...
      if (!first_iteration) {
        ...
        if (first_iteration) {
          ...
          fx->size[0] = 1;
          fx->size[1] = x->size[1];
          emxEnsureCapacity_real_T(fx, ix, &nc_emlrtRTEI);
      ...
      if (guard1) {
        ...
        y->size[0] = 1;
        y->size[1] = b_x->size[1];
        emxEnsureCapacity_real_T(y, ix, &o_emlrtRTEI);
        ix = b_x->size[1];
        for (k = 0; k < ix; k++) {
        ...
        emxEnsureCapacity_real_T(b_dv1, ix, &uc_emlrtRTEI);
        ...
        emxEnsureCapacity_real_T(b_y, ix, &o_emlrtRTEI);
        ix = b_x->size[1];
        for (k = 0; k < ix; k++) {
        ...
        emxEnsureCapacity_real_T(c_y, ix, &o_emlrtRTEI);
        ix = b_x->size[1];
        for (k = 0; k < ix; k++) {
        ...
        if (b_y->size[1] == c_y->size[1]) {
          ...
          a->size[0] = 1;
          a->size[1] = b_y->size[1];
          emxEnsureCapacity_real_T(a, ix, &vc_emlrtRTEI);
          for (ix = 0; ix < b_y->size[1]; ix++) {
            a->data[ix] = fun_workspace_be * b_y->data[ix] -
                          fun_workspace_ga * c_y->data[ix];
          }
        } else {
          binary_expand_op_6(a, fun_workspace_be, b_y, fun_workspace_ga, c_y);
        }
        ix = d_y->size[0] * d_y->size[1];
        d_y->size[0] = 1;
        d_y->size[1] = a->size[1];
        emxEnsureCapacity_real_T(d_y, ix, &o_emlrtRTEI);
        ix = a->size[1];
        for (k = 0; k < ix; k++) {
          d = a->data[k];
          d_y->data[k] = d * d;
        ...
        if (d_y->size[1] == b_dv1->size[1]) {
          ...
          emxEnsureCapacity_real_T(fx, ix, &oc_emlrtRTEI);
          for (ix = 0; ix < d_y->size[1]; ix++) {
            fx->data[ix] = d_y->data[ix] * b_dv1->data[ix] * xt->data[ix];
          }
        } else {
          binary_expand_op_4(fx, d_y, b_dv1, xt);
        }
      }
      if (first_iteration) {
        exitg1 = 1;
