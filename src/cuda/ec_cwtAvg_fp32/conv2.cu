...
static __global__ void conv2NonSeparable_kernel40(const int32_T b,
                                                  emxArray_real32_T c);

static __global__ void conv2NonSeparable_kernel41(const emxArray_real32_T c,
                                                  const int32_T b,
                                                  emxArray_real32_T b_c);

static __global__ void conv2NonSeparable_kernel42(const int32_T OH,
                                                  emxArray_real32_T expanded);

static __global__ void conv2NonSeparable_kernel43(const int32_T offsetH,
                                                  const emxArray_int32_T y,
                                                  const int32_T b_y,
                                                  emxArray_int32_T iv);

static __global__ void conv2NonSeparable_kernel44(const emxArray_real32_T a,
                                                  const emxArray_int32_T iv,
                                                  const int32_T b,
                                                  emxArray_real32_T expanded);

static __global__ void
conv2NonSeparable_kernel45(const emxArray_real32_T expanded,
...
static __global__ __launch_bounds__(1024, 1) void conv2NonSeparable_kernel41(
...
static __global__ __launch_bounds__(1024, 1) void conv2NonSeparable_kernel42(
                       ...
                       emxArray_real32_T *gpu_c, boolean_T *c_outdatedOnGpu)
{
  ...
  cpu_c->size[0] = mc;
  cpu_c->size[1] = 1;
  emxEnsureCapacity_real32_T(cpu_c, k, &mf_emlrtRTEI);
  ...
  if (validLaunchParams) {
    conv2NonSeparable_kernel40<<<grid, block>>>(mc - 1, *gpu_c);
  }
  emxInit_real32_T(&b_cpu_c, 1, &of_emlrtRTEI, true);
  emxInit_real32_T(&cpu_expanded, 2, &md_emlrtRTEI, true);
  emxInit_int32_T(&cpu_y, 2, &ud_emlrtRTEI, true);
  emxInit_int32_T(&cpu_iv, 1, &md_emlrtRTEI, true);
  emxInit_int32_T(&cpu_rows, 2, &md_emlrtRTEI, true);
  if ((cpu_a->size[0] != 0) && (cpu_b->size[0] != 0)) {
    if ((cpu_a->size[0] == 1) && (cpu_b->size[0] == 1)) {
      ...
      k = b_cpu_c->size[0];
      b_cpu_c->size[0] = 1;
      emxEnsureCapacity_real32_T(b_cpu_c, k, &nf_emlrtRTEI);
      alpha1 = 1.0F;
      beta1 = 0.0F;
      ...
      *b_outdatedOnGpu = false;
      cublasCheck(cublasSgemm(getCublasGlobalHandle(), CUBLAS_OP_N, CUBLAS_OP_N,
                              ...
                              (float *)&gpu_b->data[0], 1, (float *)&beta1,
      ...
      validLaunchParams = mwValidateLaunchParameters(grid, block);
      if (validLaunchParams) {
        conv2NonSeparable_kernel41<<<grid, block>>>(b_gpu_c, mc - 1, *gpu_c);
      }
    } else {
      real_T blockDims[3];
      int32_T b_OH;
      ...
      validLaunchParams = mwValidateLaunchParameters(grid, block);
      if (validLaunchParams) {
        conv2NonSeparable_kernel42<<<grid, block>>>(mc, gpu_expanded);
      }
      n = cpu_a->size[0];
      k = cpu_y->size[0] * cpu_y->size[1];
      cpu_y->size[0] = 1;
      cpu_y->size[1] = cpu_a->size[0];
      emxEnsureCapacity_int32_T(cpu_y, k, &nd_emlrtRTEI);
      cpu_y->data[0] = 1;
      mc = 1;
      for (k = 0; k <= n - 2; k++) {
        mc++;
      ...
      emxEnsureCapacity_int32_T(cpu_iv, k, &md_emlrtRTEI);
      mc = cpu_y->size[1] - 1;
      mwGetLaunchParameters1D(computeNumIters(mc), &grid, &block, 2147483647U);
      ...
      validLaunchParams = mwValidateLaunchParameters(grid, block);
      if (validLaunchParams) {
        conv2NonSeparable_kernel43<<<grid, block>>>(offsetH, gpu_y, mc, gpu_iv);
      }
      mc = cpu_iv->size[0];
      mwGetLaunchParameters1D(computeNumIters(mc - 1), &grid, &block,
                              2147483647U);
      ...
      validLaunchParams = mwValidateLaunchParameters(grid, block);
      if (validLaunchParams) {
        conv2NonSeparable_kernel44<<<grid, block>>>(gpu_a, gpu_iv, mc - 1,
                                                    gpu_expanded);
      }
      n = cpu_b->size[0];
      ...
      emxEnsureCapacity_int32_T(cpu_rows, k, &nd_emlrtRTEI);
      cpu_rows->data[0] = 0;
      mc = 0;
      for (k = 0; k <= n - 2; k++) {
        mc++;
      ...
      cpu_c->size[1] = 1;
      emxEnsureCapacity_real32_T(cpu_c, k, &md_emlrtRTEI);
      ...
      *b_outdatedOnGpu = false;
      validLaunchParams = mwValidateLaunchParameters(grid, block);
      if (validLaunchParams) {
        conv2NonSeparable_kernel45<<<grid, block>>>(
