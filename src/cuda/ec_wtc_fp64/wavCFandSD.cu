                  ...
                  real_T *sigmaT, real_T *cf)
{
  ...
  wname[0] = cv[static_cast<uint8_T>(wname[0]) & 127];
  if (wname[0] == 'm') {
    real_T abserrsubk;
    real_T be;
    real_T halfh;
    ...
    *sigmaT = std::sqrt(
    ...
    if (std::isinf(*sigmaT) || std::isnan(*sigmaT)) {
      ...
      if (!(b_be > 0.0)) {
        ...
        nsubs = ix - 2;
        profileLoopStart("wavCFandSD_loop_0", __LINE__, nsubs + 1, "");
        for (k = 0; k <= nsubs; k++) {
          subs[k << 1] = interval[k];
        ...
        nvtxMarkA("#emxInit_real_T#" MW_AT_LINE);
        emxInit_real_T(&b_dv1, 2, &hd_emlrtRTEI, true);
        nvtxMarkA("#emxInit_real_T#" MW_AT_LINE);
        ...
        do {
          ...
          for (k = 0; k <= nsubs; k++) {
            ...
            midpt = (d + d1) / 2.0;
            halfh = (d1 - d) / 2.0;
            profileLoopStart("wavCFandSD_loop_2", __LINE__, 14 + 1, "");
          ...
          ix = x->size[1];
          profileLoopStart("wavCFandSD_loop_3", __LINE__, (ix - 1) + 1, "");
          for (k = 0; k < ix; k++) {
            d = x->data[k];
            midpt = d / (1.0 - d);
          ...
          if (!first_iteration) {
            ...
            if (first_iteration) {
              ...
              profileLoopStart("wavCFandSD_loop_7", __LINE__,
                               (dv_idx_1 - 1) + 1, "");
          ...
          if (guard1) {
            ...
            ix = b_x->size[1];
            profileLoopStart("wavCFandSD_loop_5", __LINE__, (ix - 1) + 1, "");
            for (k = 0; k < ix; k++) {
              nvtxMarkA("#rt_powd_snf#" MW_AT_LINE);
            ...
            nvtxMarkA("#emxEnsureCapacity_real_T#" MW_AT_LINE);
            emxEnsureCapacity_real_T(b_dv1, ix, &ad_emlrtRTEI);
            profileLoopStart("wavCFandSD_loop_9", __LINE__,
                             (y->size[1] - 1) + 1, "");
            ...
            profileLoopEnd();
            ix = b_x->size[1];
            profileLoopStart("wavCFandSD_loop_10", __LINE__, (ix - 1) + 1, "");
            for (k = 0; k < ix; k++) {
            ...
            if (b_y->size[1] == b_dv1->size[1]) {
              ...
              profileLoopStart("wavCFandSD_loop_13", __LINE__,
                               (b_y->size[1] - 1) + 1, "");
          ...
          if (first_iteration) {
            ...
            midpt = 0.0;
            ix = -1;
            profileLoopStart("wavCFandSD_loop_4", __LINE__, nsubs + 1, "");
            for (k = 0; k <= nsubs; k++) {
              d = 0.0;
              d1 = 0.0;
              profileLoopStart("wavCFandSD_loop_6", __LINE__, 14 + 1, "");
              for (int32_T j{0}; j < 15; j++) {
            ...
            halfh = 0.0;
            ix = 0;
            profileLoopStart("wavCFandSD_loop_8", __LINE__, nsubs + 1, "");
            for (k = 0; k <= nsubs; k++) {
              d = errsub[k];
