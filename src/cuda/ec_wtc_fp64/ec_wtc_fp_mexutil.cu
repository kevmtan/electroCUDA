...
//
// ec_wtc_fp_mexutil.cu
//
// Code generation for function 'ec_wtc_fp_mexutil'
//
                                   ...
                                   emxArray_uint32_T *gpu, boolean_T needsCopy)
{
  ...
  int32_T totalSizeGpu;
  uint32_T *newData;
  nvtxRangePushA("#fcn#gpuEmxEnsureCapacity_uint32_T#" MW_AT_LOCATION);
  if (gpu->numDimensions == 0) {
    gpu->numDimensions = cpu->numDimensions;
    gpu->size = static_cast<int32_T *>(emlrtCallocMex(
        static_cast<uint32_T>(gpu->numDimensions), sizeof(int32_T)));
  ...
  i = 0;
  nvtxRangePushA(
      "#loop#gpuEmxEnsureCapacity_uint32_T_whileloop_0##" MW_AT_LINE);
  while (i < cpu->numDimensions) {
    totalSizeGpu *= gpu->size[i];
    totalSizeCpu *= cpu->size[i];
    gpu->size[i] = cpu->size[i];
...
{
  nvtxRangePushA("#fcn#gpuEmxFree_uint32_T#" MW_AT_LOCATION);
  if (gpu->data != (void *)4207599121UL) {
    nvtxMarkA("#checkCudaError#" MW_AT_LINE);
    nvtxMarkA("#mwCudaFree#" MW_AT_LINE);
    checkCudaError(mwCudaFree(gpu->data), __FILE__, __LINE__);
...
{
  nvtxRangePushA("#fcn#gpuEmxReset_uint32_T#" MW_AT_LOCATION);
...
// End of code generation (ec_wtc_fp_mexutil.cu)
