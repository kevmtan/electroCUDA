                ...
                int32_T olPct_size[2])
{
  ...
  for (int32_T ii{0}; ii < b_i; ii++) {
    ...
    profileLoopStart("ec_detr_fp_loop_2", __LINE__, (i4 - 1) + 1, "");
    for (int32_T ch{0}; ch < i4; ch++) {
      ...
      if (validLaunchParams) {
        ...
        ec_detr_fp_kernel6<<<grid, block>>>(ch, gpu_x, nx, gpu_dv,
                                            cpu_x->size[0U]);
      }
      ...
      if (validLaunchParams) {
        ...
        ec_detr_fp_kernel7<<<grid, block>>>(ch, gpu_w, nx, gpu_dv1,
                                            cpu_w->size[0U]);
      }
      ...
      for (int32_T b_ii{0}; b_ii < i5; b_ii++) {
        ...
        if (cpu_dv->size[0] == cpu_dv1->size[0]) {
          ...
          if (validLaunchParams) {
            ...
            ec_detr_fp_kernel8<<<grid, block>>>(gpu_dv1, gpu_dv, nx, b_gpu_x);
          }
        ...
        if (c_cpu_x->size[0] == 0) {
          ...
          profileLoopStart("ec_detr_fp_loop_4", __LINE__, (nx - 2) + 1, "");
          for (k = 0; k <= nx - 2; k++) {
        ...
        if (cpu_dv1->size[0] == 0) {
          ...
          dv1_needsGpuEnsureCapacity = true;
          profileLoopStart("ec_detr_fp_loop_5", __LINE__, (nx - 2) + 1, "");
          for (k = 0; k <= nx - 2; k++) {
        ...
        if (cpu_dv->size[0] == cpu_dv1->size[0]) {
          ...
          if (validLaunchParams) {
            ...
            ec_detr_fp_kernel9<<<grid, block>>>(gpu_dv1, gpu_dv, nx, c_gpu_x);
          }
        ...
        if (d_cpu_x->size[0] == 0) {
          ...
          profileLoopStart("ec_detr_fp_loop_7", __LINE__, (nx - 2) + 1, "");
          for (k = 0; k <= nx - 2; k++) {
        ...
        if (cpu_dv1->size[0] == 0) {
          ...
          d2scaled = cpu_dv1->data[0];
          dv1_needsGpuEnsureCapacity = true;
          profileLoopStart("ec_detr_fp_loop_8", __LINE__, (nx - 2) + 1, "");
          for (k = 0; k <= nx - 2; k++) {
            d2scaled += cpu_dv1->data[k + 1];
          }
          profileLoopEnd();
        ...
        if (cpu_dv1->size[0] == 0) {
          ...
          dv1_needsGpuEnsureCapacity = true;
          profileLoopStart("ec_detr_fp_loop_9", __LINE__, (nx - 2) + 1, "");
          for (k = 0; k <= nx - 2; k++) {
        ...
        if (cpu_regs->size[0] == cpu_dv1->size[0]) {
          ...
          if (validLaunchParams) {
            ...
            ec_detr_fp_kernel10<<<grid, block>>>(
                gpu_dv1, gpu_regs, nz, nx, b_gpu_regs, b_cpu_regs->size[0U],
                cpu_regs->size[0U]);
          }
          nvtxMarkA("#gpuEmxMemcpyGpuToCpu_real_T#" MW_AT_LINE);
        ...
        if (cpu_r->size[0] == cpu_dv1->size[0]) {
          ...
          if (validLaunchParams) {
            ...
            ec_detr_fp_kernel12<<<grid, block>>>(gpu_dv1, b_gpu_r, nz, nx,
        ...
        profileLoopStart("ec_detr_fp_loop_10", __LINE__, (nx - 2) + 1, "");
        for (i = 0; i <= nx - 2; i++) {
        ...
        k = cpu_d->size[0] - 1;
        nz = 0;
        profileLoopStart("ec_detr_fp_loop_11", __LINE__, k + 1, "");
        for (i = 0; i <= k; i++) {
        ...
        nx = 0;
        profileLoopStart("ec_detr_fp_loop_12", __LINE__, k + 1, "");
        for (i = 0; i <= k; i++) {
          if (cpu_d->data[i].re / d2scaled > 1.0E-7) {
            cpu_iv->data[nx] = i;
        ...
        if (cpu_dv->size[0] == cpu_dv1->size[0]) {
          ...
          if (validLaunchParams) {
            ...
            ec_detr_fp_kernel17<<<grid, block>>>(gpu_dv1, b_mn, gpu_dv, nx,
                                                 gpu_A);
          }
          x_outdatedOnGpu = false;
        ...
        if (cpu_dv2->size[0] == cpu_dv->size[0]) {
          ...
          if (validLaunchParams) {
            ...
            ec_detr_fp_kernel21<<<grid, block>>>(mn, gpu_dv, gpu_dv2, nx,
                                                 gpu_z);
          }
          x_outdatedOnGpu = false;
        ...
        if ((cpu_dv->size[0] == cpu_z->size[0]) && (nx == cpu_dv1->size[0])) {
          ...
          if (validLaunchParams) {
            ...
            ec_detr_fp_kernel22<<<grid, block>>>(gpu_dv1, gpu_z, gpu_dv, nx,
                                                 gpu_d);
          }
          dv2_outdatedOnCpu = false;
      ...
      if (b_cpu_x->size[0] == 0) {
        ...
        nz = b_cpu_x->data[0];
        profileLoopStart("ec_detr_fp_loop_6", __LINE__, (nx - 2) + 1, "");
        for (k = 0; k <= nx - 2; k++) {
          nz += b_cpu_x->data[k + 1];
        }
        profileLoopEnd();
