...
void sum(const emxArray_real_T *x, emxArray_real_T *y)
{
  ...
  if ((x->size[0] == 0) || (x->size[1] == 0)) {
    ...
    emxEnsureCapacity_real_T(y, xpageoffset, &b_emlrtRTEI);
    if (static_cast<int32_T>(sz_idx_1) - 1 >= 0) {
      std::memset(&y->data[0], 0, sz_idx_1 * sizeof(real_T));
    }
  } else {
    int32_T npages;
    int32_T xpageoffset;
    ...
    y->size[1] = x->size[1];
    nvtxMarkA("#emxEnsureCapacity_real_T#" MW_AT_LINE);
    emxEnsureCapacity_real_T(y, xpageoffset, &emlrtRTEI);
...
void sum(const emxArray_real_T *x, real_T y_data[], int32_T y_size[2])
{
  ...
  if ((x->size[0] == 0) || (x->size[1] == 0)) {
    ...
    for (int32_T i{0}; i < npages; i++) {
      int32_T xpageoffset;
      xpageoffset = i * x->size[0];
      if (std::isnan(x->data[xpageoffset])) {
        y_data[i] = 0.0;
      } else {
      ...
      for (int32_T k{0}; k <= vlen - 2; k++) {
        ...
        if (!std::isnan(x->data[xoffset])) {
          y_data[i] += x->data[xoffset];
        }
      }
      profileLoopEnd();
    }
