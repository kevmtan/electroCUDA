    ...
    const int32_T b, const int32_T na, emxArray_real32_T A, int32_T A_dim0)
{
  ...
  for (uint64_T idx{gThreadId}; idx <= loopEnd; idx += gStride) {
    ...
    A.data[j * A_dim0 + i] = CUDART_NAN_F;
  }
}

static __global__ __launch_bounds__(1024, 1) void mrdiv_kernel32(
    const emxArray_real32_T B, const emxArray_int32_T jpvt, const int32_T na,
    emxArray_real32_T Y)
{
  uint64_T gStride;
  uint64_T gThreadId;
...
namespace coder {
namespace internal {
void mrdiv(emxArray_real32_T *cpu_A, boolean_T *A_outdatedOnCpu,
  ...
  if ((cpu_A->size[1] == 0) || ((B->size[0] == 0) || (B->size[1] == 0))) {
    ...
    if (cpu_minmn - 1 >= 0) {
      ...
      std::memset(&cpu_A->data[0], 0,
                  static_cast<uint32_T>(cpu_minmn) * sizeof(real32_T));
    ...
    *A_outdatedOnCpu = true;
    ...
    info_t = LAPACKE_sgeqp3(102, (ptrdiff_t)b_cpu_A->size[0],
    ...
    tol = std::fmin(0.000345266977F,
                    1.1920929E-6F * static_cast<real32_T>(maxmn)) *
    ...
    gpuEmxMemcpyCpuToGpu_real32_T(&gpu_tau, cpu_tau);
    if (B_outdatedOnGpu) {
