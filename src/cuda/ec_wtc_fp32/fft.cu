...
static __global__ void fft_kernel28(const uint32_T xSize_dim1, const int32_T b,
                                    emxArray_creal32_T y);

// Function Definitions
static __global__ __launch_bounds__(1024, 1) void fft_kernel28(
    const uint32_T xSize_dim1, const int32_T b, emxArray_creal32_T y)
{
  uint64_T gStride;
  uint64_T gThreadId;
  uint64_T loopEnd;
...
//
namespace coder {
void fft(emxArray_real32_T *cpu_x, boolean_T *x_outdatedOnCpu,
         emxArray_real32_T *gpu_x, boolean_T *x_outdatedOnGpu,
         emxArray_creal32_T *cpu_y, boolean_T *y_outdatedOnCpu,
         emxArray_creal32_T *gpu_y, boolean_T *y_outdatedOnGpu)
{
  dim3 block;
  dim3 grid;
  ...
  if (cpu_x->size[1] == 0) {
    ...
    *y_outdatedOnGpu = false;
    ...
    emxEnsureCapacity_creal32_T(cpu_y, i, &ud_emlrtRTEI);
    ...
    b_fftPlanHandle =
        acquireCUFFTPlan(1, &midpoint, &midpoint, 1, 1, CUFFT_C2C, 1);
    nvtxMarkA("#gpuEmxEnsureCapacity_creal32_T#" MW_AT_LINE);
    gpuEmxEnsureCapacity_creal32_T(cpu_y, gpu_y, !*y_outdatedOnGpu);
    if (*y_outdatedOnGpu) {
      nvtxMarkA("#gpuEmxMemcpyCpuToGpu_creal32_T#" MW_AT_LINE);
      gpuEmxMemcpyCpuToGpu_creal32_T(gpu_y, cpu_y);
    }
    cufftExecC2C(b_fftPlanHandle, (cufftComplex *)&gpu_y->data[0],
                 (cufftComplex *)&gpu_y->data[0], CUFFT_FORWARD);
    *y_outdatedOnCpu = true;
    ...
    cpu_y->size[1] = cpu_x->size[1];
    nvtxMarkA("#emxEnsureCapacity_creal32_T#" MW_AT_LINE);
    emxEnsureCapacity_creal32_T(cpu_y, i, &vd_emlrtRTEI);
    fftPlanHandle =
        acquireCUFFTPlan(1, &midpoint, &midpoint, 1, 1, CUFFT_R2C, 1);
    ...
    *x_outdatedOnGpu = false;
    cufftExecR2C(fftPlanHandle, (cufftReal *)&gpu_x->data[0],
                 (cufftComplex *)&gpu_y->data[0]);
    midpoint =
    ...
    if (validLaunchParams) {
      nvtxMarkA("#fft_kernel28#" MW_AT_LINE);
      fft_kernel28<<<grid, block>>>(xSize[1], midpoint - 2, *gpu_y);
    }
    *y_outdatedOnCpu = true;
  }
  *y_outdatedOnGpu = false;
