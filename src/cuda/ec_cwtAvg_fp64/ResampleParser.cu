...
void c_ResampleParser_createFilterAn(const real_T *obj_p, const real_T *obj_q,
                                     emxArray_real_T *obj_filter,
                                     emxArray_real_T *obj_filterWithPadding,
                                     real_T *obj_filterDelay,
                                     const emxArray_real_T *obj_x)
{
  emxArray_int32_T *iv;
  ...
  b_dv[2] = 2.0 * fc;
  b_dv[3] = 1.0;
  emxInit_real_T(&r, 2, &uf_emlrtRTEI, true);
  firls(pqmax - 1.0, b_dv, r);
  emxInit_real_T(&c_dv, 1, &uf_emlrtRTEI, true);
  iseven = c_dv->size[0];
  c_dv->size[0] = r->size[1];
  emxEnsureCapacity_real_T(c_dv, iseven, &uf_emlrtRTEI);
  ...
  emxInit_real_T(&b_dv1, 1, &uf_emlrtRTEI, true);
  iseven = b_dv1->size[0];
  b_dv1->size[0] = nw;
  emxEnsureCapacity_real_T(b_dv1, iseven, &vf_emlrtRTEI);
  if (nw <= 1) {
    iseven = b_dv1->size[0];
    b_dv1->size[0] = nw;
    emxEnsureCapacity_real_T(b_dv1, iseven, &wf_emlrtRTEI);
  ...
  emxInit_real_T(&h1, 1, &xf_emlrtRTEI, true);
  iseven = h1->size[0];
  h1->size[0] = c_dv->size[0];
  emxEnsureCapacity_real_T(h1, iseven, &xf_emlrtRTEI);
  ...
  emxEnsureCapacity_real_T(obj_filter, iseven, &yf_emlrtRTEI);
  for (iseven = 0; iseven < h1->size[0]; iseven++) {
    obj_filter->data[iseven] = *obj_p * h1->data[iseven] / pqmax;
  }
  emxFree_real_T(&h1);
  ...
  *obj_filterDelay = std::floor(std::ceil(fc + ak) / *obj_q);
  ...
  emxEnsureCapacity_real_T(obj_filterWithPadding, iseven, &ag_emlrtRTEI);
  if (inw - 1 >= 0) {
    std::memset(&obj_filterWithPadding->data[0], 0,
                static_cast<uint32_T>(inw) * sizeof(real_T));
  }
  emxInit_uint32_T(&y, 2, &cg_emlrtRTEI, true);
  if (obj_filter->size[0] < 1) {
    ...
    emxEnsureCapacity_uint32_T(y, iseven, &eb_emlrtRTEI);
    for (iseven = 0; iseven < static_cast<int32_T>(u); iseven++) {
      y->data[iseven] = static_cast<uint32_T>(iseven) + 1U;
    }
  }
  emxInit_int32_T(&iv, 2, &dg_emlrtRTEI, true);
  iseven = iv->size[0] * iv->size[1];
  iv->size[0] = 1;
  iv->size[1] = y->size[1];
  emxEnsureCapacity_int32_T(iv, iseven, &bg_emlrtRTEI);
