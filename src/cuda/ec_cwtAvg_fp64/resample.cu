...
static __global__ void resample_kernel19(const emxArray_real_T varargin_1,
                                         const int32_T b_varargin_1,
                                         emxArray_real_T y);

static __global__ void resample_kernel20(const emxArray_real_T y,
                                         const int32_T b_y,
                                         emxArray_real_T varargout_1);

static __global__ void resample_kernel21(const int32_T offsetH, const int32_T b,
                                         int32_T lshift[2]);

static __global__ void resample_kernel22(const int32_T n, const int32_T offsetH,
                                         const int32_T OH, int32_T sizeX[2]);

static __global__ void resample_kernel23(const emxArray_real_T w,
                                         const int32_T y_dim0, const int32_T n,
                                         const int32_T b, const int32_T c,
                                         emxArray_real_T b_w, int32_T w_dim0);

static __global__ void resample_kernel24(const emxArray_real_T w,
                                         const int32_T y, emxArray_real_T yOut);

static __global__ void resample_kernel25(emxArray_real_T y);

static __global__ void resample_kernel26(const emxArray_real_T yOut,
                                         const emxArray_real_T y,
                                         const real_T dlast, const int32_T b,
                                         emxArray_real_T b_y);

static __global__ void resample_kernel27(const int32_T b, emxArray_real_T y);

static __global__ void resample_kernel28(const int32_T b, emxArray_real_T w);

static __global__ void resample_kernel29(const real_T absx, emxArray_real_T w);

static __global__ void resample_kernel30(const int32_T OH,
                                         emxArray_real_T expanded);

static __global__ void resample_kernel31(const int32_T offsetH,
                                         const emxArray_int32_T y,
                                         const int32_T b_y,
                                         emxArray_int32_T iv2);

static __global__ void
resample_kernel32(const emxArray_real_T opts_filterWithPadding,
...
static __global__ __launch_bounds__(1024, 1) void resample_kernel19(
    const emxArray_real_T varargin_1, const int32_T b_varargin_1,
    emxArray_real_T y)
{
  uint64_T gStride;
  uint64_T gThreadId;
...
static __global__ __launch_bounds__(1024, 1) void resample_kernel20(
    const emxArray_real_T y, const int32_T b_y, emxArray_real_T varargout_1)
{
  uint64_T gStride;
  uint64_T gThreadId;
  uint64_T loopEnd;
...
static __global__ __launch_bounds__(1024, 1) void resample_kernel21(
...
static __global__ __launch_bounds__(32, 1) void resample_kernel22(
...
static __global__ __launch_bounds__(1024, 1) void resample_kernel23(
    const emxArray_real_T w, const int32_T y_dim0, const int32_T n,
...
static __global__ __launch_bounds__(1024, 1) void resample_kernel24(
    const emxArray_real_T w, const int32_T y, emxArray_real_T yOut)
{
  uint64_T gStride;
  uint64_T gThreadId;
  uint64_T loopEnd;
...
static __global__ __launch_bounds__(32,
                                    1) void resample_kernel25(emxArray_real_T y)
{
  int32_T tmpIdx;
  tmpIdx = static_cast<int32_T>(mwGetGlobalThreadIndex());
  if (tmpIdx < 1) {
...
static __global__ __launch_bounds__(1024, 1) void resample_kernel26(
    const emxArray_real_T yOut, const emxArray_real_T y, const real_T dlast,
    const int32_T b, emxArray_real_T b_y)
{
  ...
  for (uint64_T idx{gThreadId}; idx <= loopEnd; idx += gStride) {
    int32_T orow;
    orow = static_cast<int32_T>(idx);
    b_y.data[orow] = yOut.data[static_cast<int32_T>(dlast + y.data[orow]) - 1];
  }
}

static __global__ __launch_bounds__(1024,
                                    1) void resample_kernel27(const int32_T b,
                                                              emxArray_real_T y)
{
  uint64_T gStride;
  uint64_T gThreadId;
...
static __global__ __launch_bounds__(32,
                                    1) void resample_kernel29(const real_T absx,
                                                              emxArray_real_T w)
{
  int32_T tmpIdx;
  tmpIdx = static_cast<int32_T>(mwGetGlobalThreadIndex());
...
static __global__
    __launch_bounds__(1024, 1) void resample_kernel30(const int32_T OH,
                                                      emxArray_real_T expanded)
{
  uint64_T gStride;
  uint64_T gThreadId;
...
static __global__ __launch_bounds__(1024, 1) void resample_kernel32(
    const emxArray_real_T opts_filterWithPadding, const emxArray_int32_T iv2,
              ...
              boolean_T *varargout_1_outdatedOnGpu)
{
  ...
  emxArray_real_T *cpu_opts_filterWithPadding;
  ...
  gpuEmxReset_real_T(&gpu_opts_filterWithPadding);
  w_outdatedOnGpu = false;
  y_needsGpuEnsureCapacity = true;
  emlrtHeapReferenceStackEnterFcnR2012b(emlrtRootTLSGlobal);
  emxInit_real_T(&opts_filter, 1, &pd_emlrtRTEI, true);
  emxInit_real_T(&cpu_opts_filterWithPadding, 1, &pd_emlrtRTEI, true);
  opts_filter->size[0] = 0;
  cpu_opts_filterWithPadding->size[0] = 0;
  b_cv = varargin_2 / varargin_3;
  xin = b_cv;
  absx = std::abs(b_cv);
  ...
  if (std::isinf(b_cv) || std::isnan(b_cv)) {
    if (!std::isnan(b_cv)) {
      if (b_cv < 0.0) {
        nlast = -1.0;
      } else {
    ...
    do {
      ...
      if ((b_cv == 0.0) || (std::abs(n / d - xin) <= absx)) {
        exitg1 = 1;
      } else {
        b_cv = 1.0 / b_cv;
      }
  ...
  emxInit_real_T(&cpu_y, 1, &qd_emlrtRTEI, true);
  k = cpu_y->size[0];
  cpu_y->size[0] = cpu_varargin_1->size[0];
  emxEnsureCapacity_real_T(cpu_y, k, &gd_emlrtRTEI);
  k = cpu_varargin_1->size[0] - 1;
  mwGetLaunchParameters1D(computeNumIters(k), &grid, &block, 2147483647U);
  gpuEmxEnsureCapacity_real_T(cpu_varargin_1, gpu_varargin_1,
                              !*varargin_1_outdatedOnGpu);
  gpuEmxEnsureCapacity_real_T(cpu_y, &gpu_y, true);
  if (*varargin_1_outdatedOnGpu) {
    gpuEmxMemcpyCpuToGpu_real_T(gpu_varargin_1, cpu_varargin_1);
  }
  validLaunchParams = mwValidateLaunchParameters(grid, block);
  if (validLaunchParams) {
    resample_kernel19<<<grid, block>>>(*gpu_varargin_1, k, gpu_y);
  }
  gpuEmxMemcpyGpuToCpu_real_T(cpu_y, &gpu_y);
  b_signal::internal::resample::c_ResampleParser_createFilterAn(
      &nlast, &n, opts_filter, cpu_opts_filterWithPadding, &dlast, cpu_y);
  d_opts_filterWithPadding_outdat = true;
  w_outdatedOnCpu = true;
  emxFree_real_T(&opts_filter);
  emxInit_real_T(&yCol, 1, &rd_emlrtRTEI, true);
  emxInit_real_T(&cpu_w, 2, &jd_emlrtRTEI, true);
  emxInit_real_T(&cpu_expanded, 2, &kd_emlrtRTEI, true);
  emxInit_int32_T(&b_cpu_y, 2, &sd_emlrtRTEI, true);
  emxInit_int32_T(&cpu_iv2, 1, &kd_emlrtRTEI, true);
  emxInit_real_T(&cpu_yOut, 2, &td_emlrtRTEI, true);
  emxInit_int32_T(&rows, 2, &kd_emlrtRTEI, true);
  emxInit_real_T(&c_cpu_y, 2, &ud_emlrtRTEI, true);
  emxInit_real_T(&b_cpu_w, 2, &md_emlrtRTEI, true);
  if ((!(nlast == 1.0)) || (!(n == 1.0))) {
    ...
    rndx = std::ceil(static_cast<real_T>(cpu_y->size[0]) * nlast / n);
    if (!(nlast - 1.0 >= 0.0)) {
      y = nullptr;
      ...
      emlrtAssign(&y, m);
      b_y = nullptr;
      m1 = emlrtCreateDoubleScalar(nlast - 1.0);
      emlrtAssign(&b_y, m1);
      emlrt_marshallIn(b_sprintf(y, b_y, &e_emlrtMCI), "<output of sprintf>",
                       unusedExpr);
    }
    k = yCol->size[0];
    yCol->size[0] = cpu_y->size[0] * static_cast<int32_T>(nlast);
    emxEnsureCapacity_real_T(yCol, k, &id_emlrtRTEI);
    for (offsetH = 0; offsetH < cpu_y->size[0] * static_cast<int32_T>(nlast);
         offsetH++) {
      yCol->data[offsetH] = 0.0;
    }
    if ((static_cast<int32_T>(nlast) == 0) ||
        (((static_cast<int32_T>(nlast) > 0) &&
          (cpu_y->size[0] * static_cast<int32_T>(nlast) < 1)) ||
         ((static_cast<int32_T>(nlast) < 0) &&
          (cpu_y->size[0] * static_cast<int32_T>(nlast) > 1)))) {
      b_n = 1;
      mc = 0;
    } else {
      b_n = static_cast<int32_T>(nlast);
      mc = cpu_y->size[0] * static_cast<int32_T>(nlast);
    }
    mc = div_s32(mc - 1, b_n) + 1;
    for (offsetH = 0; offsetH < mc; offsetH++) {
      yCol->data[b_n * offsetH] = cpu_y->data[offsetH];
    }
    if ((cpu_y->size[0] * static_cast<int32_T>(nlast) == 1) &&
        (cpu_opts_filterWithPadding->size[0] == 1)) {
      k = cpu_w->size[0] * cpu_w->size[1];
      cpu_w->size[0] = 1;
      cpu_w->size[1] = 1;
      emxEnsureCapacity_real_T(cpu_w, k, &jd_emlrtRTEI);
      ...
      w_needsGpuEnsureCapacity = true;
               ...
               cpu_y->size[0] * static_cast<int32_T>(nlast)) {
      if (cpu_y->size[0] * static_cast<int32_T>(nlast) == 0) {
      ...
      validLaunchParams = mwValidateLaunchParameters(grid, block);
      if (validLaunchParams) {
        resample_kernel28<<<grid, block>>>(mc - 1, gpu_w);
      }
      w_outdatedOnCpu = true;
      if (cpu_y->size[0] * static_cast<int32_T>(nlast) != 0) {
        if ((cpu_opts_filterWithPadding->size[0] == 1) &&
          ...
          cpu_w->size[1] = 1;
          emxEnsureCapacity_real_T(cpu_w, k, &jd_emlrtRTEI);
          absx = cpu_opts_filterWithPadding->data[0] * yCol->data[0];
          gpuEmxEnsureCapacity_real_T(cpu_w, &gpu_w, true);
          resample_kernel29<<<dim3(1U, 1U, 1U), dim3(32U, 1U, 1U)>>>(absx,
                                                                     gpu_w);
        } else {
          uint32_T b_OH;
          b_OH = (static_cast<uint32_T>(cpu_opts_filterWithPadding->size[0]) +
                  static_cast<uint32_T>(cpu_y->size[0] *
                                        static_cast<int32_T>(nlast))) -
                 1U;
          if (b_OH > 2147483647U) {
            b_OH = 2147483647U;
          }
          ...
          emxEnsureCapacity_int32_T(cpu_iv2, k, &kd_emlrtRTEI);
          k = b_cpu_y->size[1] - 1;
          mwGetLaunchParameters1D(computeNumIters(k), &grid, &block,
                                  2147483647U);
          ...
          validLaunchParams = mwValidateLaunchParameters(grid, block);
          if (validLaunchParams) {
            resample_kernel31<<<grid, block>>>(offsetH, c_gpu_y, k, gpu_iv2);
          }
          mc = cpu_iv2->size[0];
          mwGetLaunchParameters1D(computeNumIters(mc - 1), &grid, &block,
                                  2147483647U);
          gpuEmxEnsureCapacity_real_T(cpu_opts_filterWithPadding,
                                      &gpu_opts_filterWithPadding, false);
          gpuEmxMemcpyCpuToGpu_real_T(&gpu_opts_filterWithPadding,
                                      cpu_opts_filterWithPadding);
          validLaunchParams = mwValidateLaunchParameters(grid, block);
          if (validLaunchParams) {
            resample_kernel32<<<grid, block>>>(gpu_opts_filterWithPadding,
          ...
          rows->size[0] = 1;
          rows->size[1] = b_n;
          emxEnsureCapacity_int32_T(rows, k, &ld_emlrtRTEI);
          if (b_n > 0) {
            rows->data[0] = 0;
            mc = 0;
            for (k = 0; k <= b_n - 2; k++) {
          ...
          cpu_w->size[1] = 1;
          emxEnsureCapacity_real_T(cpu_w, k, &kd_emlrtRTEI);
          w_needsGpuEnsureCapacity = true;
          if (static_cast<int32_T>(b_OH) != 0) {
            for (offsetH = 0; offsetH < OH; offsetH++) {
              b_cv = 0.0;
              k = cpu_y->size[0] * static_cast<int32_T>(nlast);
              for (b_n = 0; b_n < k; b_n++) {
                mc = rows->data[b_n];
                ...
                if (c_opts_filterWithPadding_outdat) {
                  gpuEmxMemcpyGpuToCpu_real_T(cpu_expanded, &gpu_expanded);
                }
      ...
      int32_T c_y[2];
      c_y[0] = cpu_y->size[0] * static_cast<int32_T>(nlast);
      b_yCol = *yCol;
      d_y[0] = c_y[0];
      d_y[1] = 1;
      b_yCol.size = &d_y[0];
    ...
    resample_kernel22<<<dim3(1U, 1U, 1U), dim3(32U, 1U, 1U)>>>(b_n, offsetH, OH,
                                                               *gpu_sizeX);
    k = b_cpu_w->size[0] * b_cpu_w->size[1];
    b_cpu_w->size[0] = div_s32(offsetH, b_n) + 1;
    b_cpu_w->size[1] = mc;
    emxEnsureCapacity_real_T(b_cpu_w, k, &md_emlrtRTEI);
    k = offsetH / b_n;
    ...
    validLaunchParams = mwValidateLaunchParameters(grid, block);
    if (validLaunchParams) {
      resample_kernel23<<<grid, block>>>(gpu_w, cpu_lshift[0], b_n, k, mc - 1,
    ...
    emxEnsureCapacity_real_T(cpu_yOut, k, &nd_emlrtRTEI);
    k = cpu_sizeX[0] * cpu_sizeX[1] - 1;
    mwGetLaunchParameters1D(computeNumIters(k), &grid, &block, 2147483647U);
    gpuEmxEnsureCapacity_real_T(cpu_yOut, &gpu_yOut, true);
    c_opts_filterWithPadding_outdat = false;
    validLaunchParams = mwValidateLaunchParameters(grid, block);
    if (validLaunchParams) {
      resample_kernel24<<<grid, block>>>(b_gpu_w, k, gpu_yOut);
    }
    w_needsGpuEnsureCapacity = false;
    d_opts_filterWithPadding_outdat = true;
    absx =
        std::ceil(((static_cast<real_T>(cpu_y->size[0]) - 1.0) * nlast +
    ...
    if (std::isnan(rndx)) {
      ...
      c_cpu_y->size[0] = 1;
      c_cpu_y->size[1] = 1;
      emxEnsureCapacity_real_T(c_cpu_y, k, &eb_emlrtRTEI);
      gpuEmxEnsureCapacity_real_T(c_cpu_y, &b_gpu_y, true);
      y_needsGpuEnsureCapacity = false;
      resample_kernel25<<<dim3(1U, 1U, 1U), dim3(32U, 1U, 1U)>>>(b_gpu_y);
    } else if (rndx < 1.0) {
      c_cpu_y->size[0] = 1;
      c_cpu_y->size[1] = 0;
    } else {
      k = c_cpu_y->size[0] * c_cpu_y->size[1];
      c_cpu_y->size[0] = 1;
      c_cpu_y->size[1] = static_cast<int32_T>(rndx - 1.0) + 1;
      emxEnsureCapacity_real_T(c_cpu_y, k, &eb_emlrtRTEI);
      mwGetLaunchParameters1D(computeNumIters(static_cast<int32_T>(rndx - 1.0)),
                              &grid, &block, 2147483647U);
      gpuEmxEnsureCapacity_real_T(c_cpu_y, &b_gpu_y, true);
      y_needsGpuEnsureCapacity = false;
      validLaunchParams = mwValidateLaunchParameters(grid, block);
      if (validLaunchParams) {
        resample_kernel27<<<grid, block>>>(static_cast<int32_T>(rndx - 1.0),
                                           b_gpu_y);
      }
    }
    k = cpu_y->size[0];
    cpu_y->size[0] = static_cast<int32_T>(rndx);
    emxEnsureCapacity_real_T(cpu_y, k, &od_emlrtRTEI);
    mwGetLaunchParameters1D(computeNumIters(static_cast<int32_T>(rndx) - 1),
                            &grid, &block, 2147483647U);
    if (c_opts_filterWithPadding_outdat) {
      gpuEmxEnsureCapacity_real_T(cpu_yOut, &gpu_yOut,
    ...
    w_outdatedOnCpu = false;
    if (w_needsGpuEnsureCapacity) {
      gpuEmxMemcpyCpuToGpu_real_T(&gpu_yOut, cpu_yOut);
    }
    validLaunchParams = mwValidateLaunchParameters(grid, block);
    if (validLaunchParams) {
      resample_kernel26<<<grid, block>>>(gpu_yOut, b_gpu_y, dlast,
                                         static_cast<int32_T>(rndx) - 1, gpu_y);
    }
  }
  ...
  emxFree_real_T(&yCol);
  emxFree_real_T(&cpu_opts_filterWithPadding);
  k = cpu_varargout_1->size[0] * cpu_varargout_1->size[1];
  cpu_varargout_1->size[0] = cpu_y->size[0];
  cpu_varargout_1->size[1] = 1;
  emxEnsureCapacity_real_T(cpu_varargout_1, k, &hd_emlrtRTEI);
  k = cpu_y->size[0] - 1;
  mwGetLaunchParameters1D(computeNumIters(k), &grid, &block, 2147483647U);
  if (w_outdatedOnCpu) {
    gpuEmxEnsureCapacity_real_T(cpu_y, &gpu_y, true);
  }
  gpuEmxEnsureCapacity_real_T(cpu_varargout_1, gpu_varargout_1, true);
  validLaunchParams = mwValidateLaunchParameters(grid, block);
  if (validLaunchParams) {
    resample_kernel20<<<grid, block>>>(gpu_y, k, *gpu_varargout_1);
  }
  emxFree_real_T(&cpu_y);
  emlrtHeapReferenceStackLeaveFcnR2012b(emlrtRootTLSGlobal);
  gpuEmxFree_real_T(&gpu_opts_filterWithPadding);
