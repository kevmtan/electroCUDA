...
% Find spectral bands / keep specified frequencies
if isany(o.bands) && size(x,3)>1 
    % Spectral Band indices
    n.spect0 = n.spect; % save old spectral info
    n.spect = table;
...
elseif isany(o.freqs) && size(x,3)>1 
...
%% All-chan processing
x = cast(x,o.typeProc); % convert EEG to float type for processing
...
if o.log || o.mag2db
    ...
    if o.mag2db
        ...
        elseif n.spectOpts.fOut=="decibel"
            warning("[ec_epochBaseline] Spectral data already in decibels: "+n.sbj+" time="+toc(tt));
        else
            error("[ec_epochBaseline] Spectral data not in magnitude: "+n.sbj+" time="+toc(tt));
        end
    elseif o.log
        % Natural log
        x = log(x);
