...
if iscell(x)
    ...
    for iCell=1:numel(x)
        [x{iCell},mn{iCell}] = ec_demean(x{iCell}); % ! behaves like nt_demean2
    end
    return;
end
...
if p>1; x = ec_unfold(x); end

if isempty(w)
    mn = mean(double(x),1);
    x = nt_vecadd(x,-mn);
else
    if p>1; w = ec_unfold(w); end
