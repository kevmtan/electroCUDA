function [toGood,fromGood]=KT_interpolate_bad_channels(x,iBad,closest,d,n)
...
%  coordinates: coordinate map (see nt_proximity)chCov
...
%[closest,d]=KT_nt_proximity(coordinates);
%disp(size(closest));
%disp(size(d));
...
for iChan=iBad
    ...
    iOthers = iOthers(~ismember(iOthers,[iChan iBad])); % don't include bad channels
    iOthers = iOthers(1:n);
