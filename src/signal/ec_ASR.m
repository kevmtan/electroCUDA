...
optFields = ["unmodified","chIgnore","riemannian",...
...
%% Run ASR
if o.unmodified
    [EEG,state] = asrOg_lfn(EEG,o.refBurst,o.winSz,o.stepSz,o.dimsPCA,o.refMaxBadChs,...
        o.refTols,o.refWinSz,o.gpu,o.riemannian,o.maxMem);
else
...
splits = ceil((C*C*S*8*8 + C*C*8*S/stepSz + C*S*8*2 + S*8*5) / (maxMem*1.5 - C*C*P*8*3));
...
function [EEG,state] = asrOg_lfn(EEG,cutoff,windowlen,stepsize,maxdims,ref_maxbadchannels,ref_tolerances,ref_wndlen,usegpu,useriemannian,maxmem)
% Run the ASR method on some high-pass filtered recording.
% Signal = clean_asr(Signal,StandardDevCutoff,WindowLength,BlockSize,MaxDimensions,ReferenceMaxBadChannels,RefTolerances,ReferenceWindowLength,UseGPU,UseRiemannian,MaxMem)
%
% This is an automated artifact rejection function that ensures that the data contains no events
...
% extrapolate last few samples of the signal
sig = [EEG.data bsxfun(@minus,2*EEG.data(:,end),EEG.data(:,(end-1):-1:end-round(windowlen/2*EEG.srate)))];
...
if useriemannian
    ...
    [EEG.data,state] = asr_process(sig,EEG.srate,state,windowlen,windowlen/2,stepsize,maxdims,maxmem,usegpu);
end
