...
function [cleanData, zaplineConfig, analyticsResults, plothandles] = KT_zapline_plus(data, srate, varargin)
...
if winSizeCompleteSpectrum*srate > size(data,1)/8
    ...
    disp('Data set is short. Adjusted window size for whole data set spectrum calculation to be 1/8 of the length!')
end
...
if strcmp(noisefreqs,'line')
    ...
    noisefreqs_candidate = f(pxx_raw_log==max(pxx_raw_log(idx)));
...
if automaticFreqDetection
    ...
    verbose = 0;
    [noisefreqs,~,~,~]=findNextNoiseFreq_lfn(pxx_raw_log,f,minfreq,coarseFreqDetectPowerDiff,detectionWinsize,maxfreq,...
...
while i_noisefreq <= length(noisefreqs)
    ...
    while ~cleaningDone
        ...
        scoresC = cell(nChunks,1);
        NremoveFinal = NaN(nChunks,1);
        noisePeaks = NaN(nChunks,1);
        foundNoise = zeros(nChunks,1);
        dataC = cell(nChunks,1);
        for c = 1:nChunks
            dataC{c} = data(chunkIndices(c):chunkIndices(c+1)-1,:);
        end

        parfor c = 1:nChunks
            %%
            this_zaplineConfig_chunk = thisZaplineConfig;
            if mod(c,round(nChunks/10))==0
                disp(['Chunk ' num2str(c) ' of ' num2str(nChunks)])
            end
            chunk = dataC{c};
            %chunk = data(chunkIndices(c):chunkIndices(c+1)-1,:);

            % find flat channels and store, remove from dataset to work on
            diffchunk = diff(chunk);
            flat_channels_idx_chunk = find(all(diffchunk==0));
            if ~isempty(flat_channels_idx_chunk)
                warning(['Chunk ' num2str(c) ': Flat channels detected in chunk (will be ignored and added back in after Zapline-plus processing): ' num2str(flat_channels_idx_chunk)])
            ...
            if searchIndividualNoise
                ...
                this_freq_idx_detailed = f>noisefreq+detailedFreqBoundsUpper(1) & f<noisefreq+detailedFreqBoundsUpper(2); %#ok<PFBNS> 
                ...
                noisePeaks(c) = this_freqs_detailed(mean(pxx_chunk(this_freq_idx_detailed,:),2) == maxFinePower);
            ...
            f_noise = noisePeaks(c)/srate;

            % apply Zapline
            [cleanData_chunk,~,NremoveFinal(c),thisScores] =...
                nt_zapline_plus(chunk,f_noise,thisFixedNremove,this_zaplineConfig_chunk,0);
            scoresC{c} = thisScores;
            ...
            cleanDataC{c} = cleanData_chunk;
        end
        for c = 1:nChunks
            cleanData(chunkIndices(c):chunkIndices(c+1)-1,:) = cleanDataC{c};
            thisScores = scoresC{c};
            scores(c,1:length(thisScores)) = thisScores;
        end
    ...
    if automaticFreqDetection
        ...
        [nextfreq,~,~,~] = findNextNoiseFreq_lfn(pxx_clean_log,f,...
...
function [noisefreq,thisfreqs,thisdata,threshfound] = findNextNoiseFreq_lfn(pxx,f,minfreq,threshdiff,winsizeHz,maxfreq,lower_threshdiff,verbose)
...
if ~exist('minfreq','var')||isempty(minfreq)
    minfreq = 0;
end
if ~exist('threshdiff','var')||isempty(threshdiff)
    threshdiff = 5;
end
if ~exist('winsizeHz','var')||isempty(winsizeHz)
    winsizeHz = 3;
end
if ~exist('maxfreq','var')||isempty(maxfreq)
    maxfreq = max(f)*0.85;
end
if ~exist('lower_threshdiff','var')||isempty(lower_threshdiff)
