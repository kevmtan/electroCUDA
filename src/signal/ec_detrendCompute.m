function [x,w,r] = ec_detrendCompute(x,order,thresh,niter,wsize,w,basis) %,doubleIn,singleOut)
...
arguments
    ...
    wsize {isnumeric} = 0          % Overlapping window size (default=[])
    w logical = true(size(x))      % Pre-calculated weights
...
if ~isany(wsize)
    ...
    [x,w,r] = ec_detrend_lfn(x,order,thresh,niter,w,basis);
else
    % Overlapping window detrending
    [x,w] = ec_detrendWin_lfn(x,order,thresh,niter,wsize,w,basis);
    r = [];
end
...
%% Standard detrending (trend fit to entire data) %%%%%%%%%%%%%%%%%%%%%%%%%
function [y,w,r] = ec_detrend_lfn(x,order,thresh,niter,w,basis)
...
switch basis
    case 'polynomials'
        r = coder.nullcopy(x(:,1:order)); r(:)=0; % r=zeros(dims(1),numel(order));
        lin = linspace(-1,1,dims(1));
        for k=1:order
            r(:,k) = lin.^k;
        end
    case 'sinusoids'
        r = coder.nullcopy(x(:,1:order*2)); r(:)=0; %r=zeros(dims(1),numel(order)*2);
        lin = linspace(-1,1,dims(1));
        for k = 1:order
...
y = coder.nullcopy(x); y(:)=0; %zeros(size(x));
for ii = 1:niter
    ...
    ww = true(dims);
    ww(abs(d) > thresh * repmat(std(d),dims(1),1)) = false;
...
function [y,w] = ec_detrendWin_lfn(x,order,thresh,niter,wsize,w,basis)
dims = size(x);
wsize = 2*floor(wsize/2);
y = coder.nullcopy(x); y(:)=0;

% 1) divide into windows, 2) detrend each, 3) stitch together, 4) estimate w
for iIter = 1:niter
    y = coder.nullcopy(x); y(:)=0; %y = zeros(size(x));
    trend = coder.nullcopy(y); %trend = zeros(size(x));
    a = coder.nullcopy(y(:,1)); %zeros(dims(1),1);
    offset = 0;
    while true
        start = offset+1;
        ...
        % detrend this window
        NITER = 1;
        yy = ec_detrend_lfn(x(start:stop,:),order,thresh,NITER,w(start:stop,:),basis);
...
if size(x,1) ~= size(y,1)
    error("ec_detrendCompute/regw_lfn: x & y are unequal sizes");
end
...
if size(w,2)==1
    ...
    [V,D] = eig(xx'*xx);
    V = real(V);
    topcs = V(:,D/max(diag(real(D))) > thr_PCA); % discard weak dims
    xxx = xx*topcs;
    b = ((yy'*xxx) / (xxx'*xxx))';
    ...
    b = zeros(flip(size(y)));
    for ch = 1:size(y,2)
        if sum(w(:,ch))==0; disp("All weights zero: ch"+ch); continue; end
...
if isempty(w)
    ...
    if size(w,2)==1
        mn = sum(x.*w,1) ./ (sum(w,1)+eps); %sum(vecmult_lfn(x),w),1) ./ (sum(w,1)+eps);
    elseif size(w,2)==n
...
if ~isempty(x)
    ...
    if nepochs~=round(nepochs)
        ...
        x = x(1:N*nepochs,:);
        disp('nsamples not multiple of epoch size, truncating...');
    end
    %if nepochs>1
