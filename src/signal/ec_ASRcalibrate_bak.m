function state = ec_ASRcalibrate(X,fs,cutoff,blockSz,filtHz,filtMag,...
    winLength,winOverlap,maxDropout,minClean,doGPU)
...
arguments
    ...
    fs (1,1) double
    cutoff (1,1) double = 5
    blockSz (1,1) double = 10 % Block size of calculations in samples
    filtHz (1,:) double =  [0  2   3   13 16 40 80 fs/2] % Filter frequencies
    filtMag (1,:) double = [3 .75 .33 .33 1  1  3  3]    % Filter magnitude
    winLength (1,1) double = 1 % Window length (secs) of artifact detection
    winOverlap (1,1) double = 2/3 % fraction of two successive windows that overlaps
    maxDropout (1,1) double = 0.1 % Max pct of wins with signal dropouts
    minClean (1,1) double = 0.25 % Min pct of wins that need to be clean for thresh estimates
...
if ~exist("iirstate","var"); iirstate="NO FILTERING"; disp(iirstate); end
...
med = geoMedianBlock_lfn(U/blockSz,1,doGPU);
if isgpuarray(med); med=gather(med); end
if isnan(med(1)) && size(U,2)==1; med=median(U); end
M = sqrtm(real(reshape(med,C,C)));

% Window length for calculating thresholds
N = round(winLength*fs);
if S < N
...
for c = C:-1:1
    ...
    rms = X(:,c).^2;
    rms = sqrt(sum(rms(bsxfun(@plus,round(1:N*(1-winOverlap):S-N),(0:N-1)')))/N); % rms = sqrt(sum(rms(round(1:N*(1-winOverlap):S-N) + (0:N-1)'))/N);
    % fit a distribution to the clean part
    [mu(c),sig(c)] = ec_fitEEGdist(rms,minClean,maxDropout);
end
...
%% geoMedianBlock_lfn
function X = geoMedianBlock_lfn(X,blockSz,doGPU)
...
if blockSz > 1
    ...
    if r > 0
        X = [reshape(sum(reshape(X(1:(o-r),:),blockSz,b*v)),b,v);...
            sum(X((o-r+1):end,:))*(blockSz/r)];
    else
...
if doGPU
    %try
    %    X = gather(geomMedian_lfn(gpuArray(X),tol,maxIter)) / blockSz;
    %    fin = 1;
    %catch ME; getReport(ME)
    %    disp("switching to CPU for: ec_ASRcalibrate/geomMedian_lfn");
    %end
end
if ~fin
    X = geomMedian_lfn(X,tol,maxIter) / blockSz;
end



%% geoMedian_lfn %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function y = geomMedian_lfn(X,tol,max_iter)
...
y = median(X);
for ii = 1:max_iter
    invnorms = 1./sqrt(sum(bsxfun(@minus,X,y).^2,2)); % = 1./ sqrt(sum((X-y).^2,2));
    oldy = y;
    y = sum(X.*invnorms) / sum(invnorms);
