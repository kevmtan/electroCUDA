...
% sbj="S15_87_RL"; task="MMR"; proj="lbcn"; a.saveFig=0; a.targetHz=1000; r=1;
...
for t = tFirst+1:height(sot)
    ...
    if df <= 0.02
        sot.pd(t) = onsStim(idx); end % Only use if within 30ms
end
...
plot(sot.pd,repmat(0.2,height(sot),1),"*",Color=[1 0 0],DisplayName="Pdio");
...
trialinfo.onsFin(sot.trial) = sot.fin;
trialinfo.onsFin(tRest) = trialinfo.onsFin(tRest-1) + trialinfo.durBeh(tRest-1);
