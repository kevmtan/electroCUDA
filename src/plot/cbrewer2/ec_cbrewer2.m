...
if isempty(ncol)
    ...
    ncol = 256; %size(get(gcf,'colormap'), 1);
end
...
if ncol > length(colorbrewer.(ctype).(cname))
    ...
    cmap = colorbrewer.(ctype).(cname){length(colorbrewer.(ctype).(cname))};
    cmap = cmap ./ 255;
elseif isempty(colorbrewer.(ctype).(cname){ncol})
    ...
    cmap = colorbrewer.(ctype).(cname){nmin};
    cmap = cmap./255;
else
    % If we specified a number of colours in the pre-designed range, no
    % need to interpolate
    cmap = (colorbrewer.(ctype).(cname){ncol}) ./ 255;
    return;
end
...
if strcmp(ctype,'qual')
    ...
    % return
    cmap = repmat(cmap, ceil(ncol / size(cmap, 1)), 1);
    cmap = cmap(1:ncol, :);
    return;
end
...
if ~strcmpi(interp_space,'rgb')
    cmap = colorspace(['rgb->' interp_space], cmap);
end

% Linearly interpolate
X  = linspace(0, 1, size(cmap, 1));
XI = linspace(0, 1, ncol);
cmap = interp1(X, cmap, XI, interp_method);
